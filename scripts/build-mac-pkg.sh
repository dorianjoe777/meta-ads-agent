...
BUILD_DIR="$RELEASE_DIR/pkg-build"
PKG_ROOT="$BUILD_DIR/root"
APP_DIR="$PKG_ROOT/Applications/$APP_NAME"
COMPONENT_PKG="$BUILD_DIR/$APP_NAME-component.pkg"
FINAL_PKG="$RELEASE_DIR/MetaAdsAgent-$VERSION-mac.pkg"

if ! command -v pkgbuild >/dev/null 2>&1 || ! command -v productbuild >/dev/null 2>&1; then
  echo "No encontre pkgbuild/productbuild. Este script debe correrse en macOS con Xcode Command Line Tools."
  exit 1
fi
...
python3 - "$APP_DIR/installer/release-bootstrap.env" <<'PY'
import os
import sys
...
chmod +x "$APP_DIR/Instalar en Mac.command" || true
chmod +x "$APP_DIR/Instalar en Linux.sh" || true
