...
class ReleasePreflightTests(unittest.TestCase):
    def test_script_is_executable_and_read_only_by_contract(self):
        ...
        self.assertIn("014_telegram_typing_indicator.sql", text)
        self.assertIn("015_telegram_typing_retry_continuity.sql", text)
        self.assertIn("telegram_update_pending", text)
        self.assertIn("Telegram typing indicator stays visible through runtime retries with ingress-only access", text)
        ...
        self.assertIn("DASHBOARD_STATUS.md", text)
        ...
        self.assertIn("017_licensed_central_image_pool_switch.sql", text)
        self.assertIn("019_trial_grace_deletion_fencing.sql", text)
        self.assertIn("claim_grace_deletion_candidates", text)
        self.assertIn("scheduler_broker_key", text)
        self.assertIn("runtime broker has a private scheduler-only credential", text)
        self.assertIn("control image revision matches DEPLOYED_COMMIT", text)
        self.assertIn("control image tag/build SHA must match the exact DEPLOYED_COMMIT", text)
        self.assertIn("licensed_central_image_pool_enabled", text)
        self.assertIn("operator_set_licensed_central_image_pool", text)
        self.assertIn("licensed accounts use an explicit central-pool switch", text)
        self.assertIn("licensed central-pool switch is explicit, operator-only", text)
