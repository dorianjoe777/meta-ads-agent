...
class IntegrationTestSuite:
    ...
    def test_release_package_excludes_runtime_data_and_includes_buyer_docs(self):
        ...
        self.assert_true("if(!data.valid){\n          if(pendingWindow) pendingWindow.close();" in portal_page and "shouldAskForFreshDigitalOceanToken(data)" in portal_page and "refresh_access_failed" in portal_page, "Cloud dashboard opener shows portal refresh errors and asks for a fresh token instead of hiding them behind the Droplet access gate")
        ...
        self.assert_true("Firewalls: actualizar y Droplets: leer" in portal_digitalocean_api and "aunque el scope de firewall exista" in portal_digitalocean_api, "DigitalOcean 403 firewall errors explain the attached-Droplet read scope instead of saying the token is invalid")
