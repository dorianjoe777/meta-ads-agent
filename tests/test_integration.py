...
Perfecto. Ya entendí que tienes algo de experiencia con anuncios. Ahora cuéntame qué vende Singularity X Récords y cuál es su meta principal."""
    ...
    def test_dashboard_image_only_chatgpt_connect_preserves_text_brain(self):
        ...
        original_log = dashboard.log_action
        try:
            dashboard.load_config = lambda: type("Cfg", (), {
                "agent_brain_provider": "minimax",
                "agent_chat_provider": "hermes",
                "agent_chat_base_url": "https://api.minimax.io/v1",
                "hermes_home": str(ROOT_DIR / "dashboard" / "data" / "hermes-home"),
    ...
    def test_dashboard_image_connect_reuses_main_chatgpt_brain(self):
        """Test Image 2 does not switch to a stale dedicated session when ChatGPT/Codex is primary."""
        print("\nTesting Dashboard Image Connect Reuses Main ChatGPT Brain...")
        ...
        try:
            dashboard.load_config = lambda: type("Cfg", (), {
                ...
                "codex_image_hermes_home": str(ROOT_DIR / "dashboard" / "data" / "hermes-image-home"),
                "codex_image_hermes_model": "gpt-5.5",
            })()
            dashboard.update_env_values = lambda values: captured.update(values)
            dashboard.hermes_codex_ready = lambda _config: (True, "Provider: OpenAI Codex; OpenAI Codex ✓ logged in")
            ...
            result = dashboard.connect_agent_model({"connection_purpose": "image", "codex_image_hermes_model": "gpt-5.5"})
            disconnected = dashboard.disconnect_agent_model({"connection_purpose": "image"})

            self.assert_true(result["status"] == "completed" and result.get("mode") == "image_uses_main_chatgpt", "Image connect reports that Image 2 reuses the primary ChatGPT/Codex session")
            self.assert_true(captured == {}, "Image connect does not persist dedicated image routing when ChatGPT/Codex is already the primary brain")
            self.assert_true(disconnected["status"] == "using_main_chatgpt" and disconnected.get("removed") == [], "Image disconnect cannot accidentally clear the primary ChatGPT/Codex auth home")
        finally:
            dashboard.load_config = original_load
            dashboard.update_env_values = original_update
            dashboard.hermes_codex_ready = original_ready
    ...
    def test_dashboard_chatgpt_connect_action_uses_vps_browserless_bridge(self):
        ...
        try:
            ...
            self.assert_true("AGENT_CHAT_PROVIDER" not in captured and "AGENT_BRAIN_PROVIDER" not in captured, "VPS ChatGPT login is saved as a connection without silently replacing the primary provider")
        finally:
            dashboard.update_env_values = original_update
            dashboard.launch_hermes_terminal = original_launch
            dashboard.start_hermes_browserless_login = original_start
    ...
    def test_hermes_product_skills_are_copied_to_workspace(self):
        ...
        try:
            ...
            self.assert_true("Never update Hermes, MCP" in agents_text and "Never update Hermes, MCP" in standalone_skills_text, "Hermes may diagnose incompatibilities but cannot self-update dependencies or product compatibility code")
    ...
    def test_hermes_gateway_redacts_token_and_handles_start_failure(self):
        ...
        try:
            ...
            popen_calls.clear()
            os.environ.update({
                "ADMIRA_MINIMAX_API_KEY": "secondary-minimax-key",
                "ADMIRA_MINIMAX_BASE_URL": "https://api.minimax.io/v1",
                "ADMIRA_MINIMAX_MODEL": "MiniMax-M3",
                "ADMIRA_MINIMAX_API": "openai-chat-completions",
                "ADMIRA_NVIDIA_API_KEY": "secondary-nvidia-key",
                "ADMIRA_NVIDIA_BASE_URL": "https://integrate.api.nvidia.com/v1",
                "ADMIRA_NVIDIA_MODEL": "z-ai/glm-5.2",
                "ADMIRA_NVIDIA_API": "openai-chat-completions",
            })
            ...
            self.assert_true(secondary_process_env.get("ADMIRA_MINIMAX_API_KEY") == "secondary-minimax-key" and secondary_process_env.get("ADMIRA_NVIDIA_API_KEY") == "secondary-nvidia-key", "Hermes Gateway passes every saved endpoint credential for manual Telegram /model switches")
            ...
            self.assert_true('provider: "openai-codex"' in secondary_config and "admira-minimax:" in secondary_config and "admira-nvidia:" in secondary_config and '"minimax m3":' in secondary_config and '"nvidia nim":' in secondary_config, "Telegram /model lists all saved providers while ChatGPT/Codex remains the single primary")
            self.assert_true("secondary-minimax-key" not in secondary_config and "secondary-nvidia-key" not in secondary_config, "Secondary provider secrets never leak into config.yaml")
    ...
    def test_hermes_daily_brief_cron_edge_cases(self):
        ...
        try:
            ...
            self.assert_true("Si la búsqueda devuelve filtros sugeridos" in social_prompt and "no se los devuelvas al comprador" in social_prompt and "elige tú" in social_prompt, "Motion workflow makes reversible recipe choices proactively instead of asking the buyer to operate the storyboard")
    ...
    def test_hermes_business_memory_workspace_is_curated_and_redacted(self):
        ...
        self.assert_true("purposeful emojis" in soul_text and "📊" in soul_text and "⚠️" in soul_text and "childish" in soul_text, "Hermes soul uses emojis as restrained visual anchors instead of decoration")
    ...
    def test_codex_image_cli_bridge_copies_generated_asset(self):
        ...
        try:
            ...
            codex_brand_guides.run_hermes_image_bridge = fake_reference_unsupported_bridge
            codex_brand_guides.subprocess.run = fake_direct_run
            main_routed = codex_brand_guides.call_codex_image_cli(
                "Usa la referencia adjunta con la sesión principal de ChatGPT",
                output_root=test_root / "creatives-main-chatgpt",
                output_name="direct-main-chatgpt",
                reference_image_paths=[reference_image],
            )
            self.assert_true(main_routed["ok"] is True and all(call[1].get("CODEX_HOME") == hermes_home and call[1].get("HERMES_HOME") == hermes_home for call in direct_calls), "Codex/Image ignores stale dedicated image auth when ChatGPT/Codex is the primary brain")

            bridge_env.clear()
            codex_brand_guides.run_hermes_image_bridge = original_bridge
            codex_brand_guides.subprocess.run = fake_bridge_run
            codex_brand_guides.run_hermes_image_bridge({"mode": "status"}, config=codex_brand_guides.load_config())
            self.assert_true(bridge_env.get("HERMES_HOME") == hermes_home, "Codex/Image Hermes bridge reuses the primary ChatGPT/Codex home when ChatGPT/Codex is the primary brain")
        finally:
            codex_brand_guides.run_hermes_image_bridge = original_bridge
    ...
    def test_brand_memory_documents_feed_creative_generation(self):
        ...
        try:
            ...
            self.assert_true(logo_upload["library"]["general"]["fields"]["logo_path"] == logo_upload["logo_path"] and logo_upload["logo_path"].startswith("brand_guides/assets/luz-clara-logo-"), "Dashboard logo upload keeps the newly saved official logo selected for future creatives")
    ...
    def test_demo_metrics_are_labeled(self):
        ...
        try:
            ...
            self.assert_true("emojis funcionales" in simple_context["communication_preference"]["instruction"] and "📊" in simple_context["communication_preference"]["instruction"] and "no en cada frase" in simple_context["communication_preference"]["instruction"], "Simple-language runtime context asks for restrained visual emoji signposts")
    ...
    def test_setup_page_contains_unlock_and_trust(self):
        ...
        self.assert_true("activation-footer" in html and "Tus claves permanecen en esta instalación." in html, "One-page activation keeps a short local-secret reassurance")
        ...
        self.assert_true("doneState.completed&&!doneState.requires_repair" in html, "Legacy completed setup reopens the one-page activation when real data needs repair")
        ...
        self.assert_true("Conecta el cerebro del agente" in html and "MiniMax M3" in html and "Guardar conexión" in html and "Usar como principal" in html, "Agent model setup separates saving a connection from selecting the single primary brain")
        ...
        self.assert_true("const brain=model.brain_provider||'nvidia_nim'" in html and "primaryRoute=brain==='openai_codex'?'chatgpt_subscription'" in html, "Fresh onboarding starts with NVIDIA NIM while keeping ChatGPT as an explicit alternative")
        ...
        self.assert_true('option value="openai_codex"' in html and "activation-model-hint" in html and "ChatGPT suscripción" in html and "Conectar tu suscripción de ChatGPT" in html, "One-page activation exposes ChatGPT subscription as a primary model, separate from image-only ChatGPT")
        self.assert_true("ChatGPT Plus o superior" in html and "una cuenta Free no sirve como modelo principal" in html and 'name="hermes_model" value="${escapeHtml(compactHermesModel)}"' in html and "gpt-5.6-luna" in html and "Luna se elegirá automáticamente" in html, "One-page ChatGPT activation explains the Plus-or-higher requirement and pins Luna as the automatic model")
        ...
        self.assert_true("data-input-code=\"autoSaveTelegramToken(event)\"" in html and "telegram-token-zone" in html and "telegram-token-saved-inline" in html and "Clave guardada" in html and "Ahora abre el bot que creaste" in html and "telegram-detect-button" in html and "detectTelegramChats()" in html and "telegramStatusFromResponse" in html and "response?.result" in html and "send_welcome:'true'" in html and "Detecté tu chat" in html and "Usar este chat y enviarme el primer mensaje" not in html, "Telegram token autosave unwraps the API result, shows detection clearly, and selects the detected chat without another buyer click")
        self.assert_true("startTelegramHelloPolling" in html and "detectTelegramChats({silent:true})" in html and "setTimeout(async()=>{" in html and "2200" in html, "One-page activation continuously detects the buyer hello without a manual detect button")
        ...
        self.assert_true("maybeFinishTelegramOnboarding" in html and "steps.every(step=>step.status==='ok')" in html and "finishOnboardingAndStartTour('telegram')" in html, "A detected Telegram hello automatically completes activation once every required section is ready")
        ...
        self.assert_true("if(!licenseOk)steps.push" not in html and "La licencia se corrige desde el instalador o con soporte; nunca se pega aquí." in html, "License stays installer-managed and is not requested in the one-page activation")
        ...
        self.assert_true("{id:'password',status:passwordOk?'ok':'blocked'}" in html and "compactPasswordSetup(steps[0].status==='ok')" in html, "Clean installs show password creation as the first one-page activation section")
        ...
        self.assert_true("activation-footer" in html and "Revisar y abrir dashboard" in html, "One-page activation keeps one final review action at the bottom")
        ...
        self.assert_true("maybeAutoDiscoverCompactSetup" in html and "discoverMetaAssets(v.ad_account_id)" in html, "The combined Meta section automatically discovers Pages after an ad account is selected")
        ...
        self.assert_true("const destinationOk=setupItem('page_id').status==='ok'" in html, "Onboarding requires the selected Page but not an existing ad set or destination website")
        ...
        self.assert_true("Saltar y completar luego" not in html[html.find('function renderOnboardingFlow()'):html.find('function maybeAutoDiscoverDestination')], "Required one-page activation no longer encourages skipping the connection essentials")
    ...
    def test_setup_config_save_preserves_blank_license(self):
        ...
        try:
            ...
            self.assert_true("ADMIRA_MINIMAX_API_KEY=direct-model-key" in env_after and "ADMIRA_MINIMAX_MODEL=MiniMax-M3" in env_after, "The primary MiniMax endpoint is also retained as an independently saved connection")
            ...
            nvidia_result = dashboard.save_setup_config(
                {
                    ...
                    "agent_model_action": "save_connection",
                    "agent_chat_base_url": "https://integrate.api.nvidia.com/v1",
                    "agent_chat_model": "z-ai/glm-5.2",
                    "agent_chat_api": "openai-chat-completions",
                    "agent_chat_api_key": "nvapi-direct-model-key",
            ...
            saved_connections = product_config.agent_model_connections(reloaded)
            self.assert_true("ADMIRA_NVIDIA_API_KEY=nvapi-direct-model-key" in nvidia_env and "ADMIRA_NVIDIA_MODEL=z-ai/glm-5.2" in nvidia_env, "Saving NVIDIA stores that endpoint independently")
            self.assert_true("AGENT_BRAIN_PROVIDER=minimax" in nvidia_env and reloaded.agent_brain_provider == "minimax", "Saving a second endpoint does not silently replace the primary brain")
            self.assert_true(saved_connections["minimax"]["configured"] and saved_connections["minimax"]["primary"] and saved_connections["nvidia_nim"]["configured"] and not saved_connections["nvidia_nim"]["primary"], "Dashboard can show multiple connected endpoints with exactly one primary")
            self.assert_true("api_key" not in saved_connections["nvidia_nim"] and saved_connections["nvidia_nim"]["api_key_set"], "Saved connection payload reports key presence without exposing the secret")
            self.assert_true(gateway_refreshes == ["minimax", "minimax"] and nvidia_result.get("gateway", {}).get("started") is True, "Saving a secondary endpoint refreshes Telegram's model catalog without changing its primary")

            primary_result = dashboard.save_setup_config(
                {
                    "agent_chat_provider": "nvidia_nim",
                    "agent_model_action": "set_primary",
                    "agent_chat_base_url": "https://integrate.api.nvidia.com/v1",
                    "agent_chat_model": "z-ai/glm-5.2",
                    "agent_chat_api": "openai-chat-completions",
                    "agent_chat_api_key": "",
            ...
            primary_env = env_path.read_text(encoding="utf-8")
            primary_config = dashboard.load_config()
            primary_connections = product_config.agent_model_connections(primary_config)
            self.assert_true("AGENT_BRAIN_PROVIDER=nvidia_nim" in primary_env and "AGENT_CHAT_API_KEY=nvapi-direct-model-key" in primary_env, "Use as primary reuses the saved key without making the buyer paste it again")
            self.assert_true(primary_config.agent_brain_provider == "nvidia_nim" and primary_connections["nvidia_nim"]["primary"] and not primary_connections["minimax"]["primary"], "Selecting NVIDIA moves the single primary marker and preserves MiniMax as connected")
            self.assert_true(gateway_refreshes == ["minimax", "minimax", "nvidia_nim"] and primary_result.get("gateway", {}).get("started") is True, "Using a saved endpoint as primary restarts Telegram on that endpoint")
        finally:
            env_path.write_text(env_before, encoding="utf-8")
            ad_path.write_text(ad_before, encoding="utf-8")
            if onboarding_before:
    ...
    def test_release_package_excludes_runtime_data_and_includes_buyer_docs(self):
        ...
        self.assert_true("CODEX_CLI_VERSION=0.142.5" in dockerfile and "HERMES_AGENT_VERSION=0.18.0" in dockerfile and "MCP_SDK_VERSION=2.0.0" in dockerfile and "@openai/codex@${CODEX_CLI_VERSION}" in dockerfile and '"hermes-agent==${HERMES_AGENT_VERSION}"' in dockerfile and '"mcp==${MCP_SDK_VERSION}"' in dockerfile, "Fresh Docker installs pin the smoke-tested Hermes, MCP, and Codex builds instead of pulling main/latest drift")
        ...
        self.assert_true('CODEX_CLI_VERSION="${CODEX_CLI_VERSION:-0.142.5}"' in install_local and 'HERMES_AGENT_VERSION="${HERMES_AGENT_VERSION:-0.18.0}"' in install_local and 'MCP_SDK_VERSION="${MCP_SDK_VERSION:-2.0.0}"' in install_local, "Native installs use the same tested dependency pins with explicit override support")
        ...
        entrypoint = (ROOT_DIR / "scripts" / "docker-entrypoint.sh").read_text(encoding="utf-8")
        self.assert_true("Migrated existing Hermes connection" in entrypoint and 'legacy_hermes_home="/app/dashboard/data/hermes-home"' in entrypoint and '[ ! -f "$runtime_hermes_home/config.yaml" ]' in entrypoint, "Docker updates migrate a legacy connected Hermes home before isolating runtime state")
        ...
        self.assert_true("ADMIRA_ALLOW_LOCAL_SOURCE_INSTALL" in windows_installer and "ADMIRA_ALLOW_LOCAL_SOURCE_INSTALL" in mac_installer and "ADMIRA_ALLOW_LOCAL_SOURCE_INSTALL" in linux_installer and "No voy a iniciar una copia local" in windows_installer, "Failed protected updates never fall back to a local compose run that could replace another installation")
        ...
        self.assert_true('com.docker.compose.project' in bootstrap_sh and '/app/runtime' in bootstrap_sh and 'detected_runtime_volume%_config' in bootstrap_sh, "Linux updates recover legacy project and persistent-volume names from the running container")
        self.assert_true('com.docker.compose.project' in windows_bootstrap and '/app/runtime' in windows_bootstrap and "-replace '_config$'" in windows_bootstrap, "Windows updates recover legacy project and persistent-volume names from the running container")
        self.assert_true("dashboard_html_document" in dashboard_server_source and "current_product_version()" in dashboard_server_source and "dashboard.js?v={version}" in dashboard_server_source, "Dashboard asset URLs change with each release so buyers do not keep stale model-picker JavaScript")
        ...
        self.assert_true("CreateShortcut" in nsis_template and "wscript.exe" in nsis_template and "Abrir Admira IA.vbs" in nsis_template, "Windows NSIS installer creates a buyer shortcut that opens the app without rerunning installation")
        env_example = (ROOT_DIR / ".env.example").read_text(encoding="utf-8")
        ...
        self.assert_true("Buscar update ahora" in dashboard_js_source and "checkForUpdates(true)" in dashboard_js_source and "UPDATE_CHECK_COOLDOWN_MS" in dashboard_js_source and "updateCheckInFlight" in dashboard_js_source, "Dashboard lets buyers manually force an update check and avoids blocking all checks after the first tab check")
        self.assert_true("if(updateCheckStarted&&!force)return" not in dashboard_js_source, "Dashboard update checks are not permanently suppressed after the first automatic check in an open tab")
        ...
        self.assert_true("update-cards" in dashboard_js_source and "showUpdateDetails" in dashboard_js_source and "Actualización oficial" in dashboard_js_source, "Dashboard shows update improvements as cards before installing")
        ...
        self.assert_true("AUTOMATIC_UPDATE_STATE_FILE" in dashboard_server_source and "run_scheduled_automatic_update" in dashboard_server_source and "3 <= local_now.hour < 9" in dashboard_server_source and "send_scheduled_automatic_update_notice" in dashboard_server_source and '"scheduled_hour": 9' in dashboard_server_source and "ensure_automatic_update_monitor()" in dashboard_server_source, "Updates run once in the buyer-local 3 AM maintenance window and send one 9 AM success notice")
        self.assert_true("Las actualizaciones oficiales se instalan automáticamente a las 3:00 a. m." in dashboard_js_source and "UPDATE_CHECK_POLL_MS=60*1000" in dashboard_js_source and "startLiveMetricsAutoRefresh();startUpdateAutoCheck()" in dashboard_js_source and "checkForUpdates(false,{silent:true})" in dashboard_js_source, "Dashboard detects releases while open without installing routine updates")
        ...
        self.assert_true("data-account-choice" in dashboard_js_source and "selectSocialAccountFromElement" in dashboard_js_source and "bindAccountChoiceButtons" in dashboard_js_source, "Meta account switch buttons keep their payload in a safe data attribute and direct click fallback so changing a key cannot render an inert button")
        ...
        self.assert_true('entitlements.plan === "individual"' in device_binding_lib and "ensureDeviceBinding" in license_activate_api and "ensureDeviceBinding" in license_release_api, "License server restricts device transfer to Individual licenses through the shared binding policy")
        ...
        self.assert_true("Mis instalaciones" in portal_page and "renderPortfolio" in portal_page and "data-installation-token" in portal_page, "Buyer access area manages multiple isolated installations from one email")
        self.assert_true("buyerPortfolio" in portal_session_api and "readRegistry" in portal_session_api and 'action === "select_license"' in portal_session_api and 'action === "rename_installation"' in portal_session_api, "Portal session discovers same-buyer licenses and securely switches or renames them")
        self.assert_true("switchToken" in portal_session_api and "data-license-key" not in portal_page, "Portfolio switching uses signed tokens instead of exposing full license keys in the page")
        ...
        self.assert_true("install_event" in license_activate_api and "onboarding_opened" in device_binding_lib and "onboarding_completed" in device_binding_lib, "License activation records local onboarding state for the buyer portal through the shared binding policy")
        ...
        self.assert_true("US$6 al mes" in portal_page and "credito inicial" in portal_page and "metodo de pago" in portal_page, "Cloud install explains the current 1GB DigitalOcean minimum cost and signup requirements")
        ...
        self.assert_true("AbortController" in portal_page and "request_timeout" in portal_page and "install_request_id" in portal_page and "No pulses crear otra vez" in portal_page, "Cloud creation has a bounded browser request, a stable idempotency key, and a safe retry message instead of freezing near the first progress value")
        self.assert_true("DIGITALOCEAN_DROPLET_TIMEOUT_MS" in portal_digitalocean_api and "findDropletByTag" in portal_digitalocean_api and "cloud_request_timeout" in portal_digitalocean_api, "DigitalOcean creation times out safely, adopts a completed Droplet by tag, and never creates a duplicate after a lost response")
        self.assert_true("install_job_id" in portal_digitalocean_api and "install_progress: Math.max" in portal_digitalocean_api and "cloud_state_persist_failed" in portal_digitalocean_api and "pendingSaved" in portal_digitalocean_api, "Cloud installs persist an owned pending job before the Droplet request and clean up if durable state cannot be saved")
