...
class IntegrationTestSuite:
    ...
    def test_setup_page_contains_unlock_and_trust(self):
        ...
        self.assert_true("{id:'telegram',status:telegramOk?'ok':'warn'}" in html and "telegramOnboardingGuide()" in html, "Initial onboarding connects Telegram before the final communication preference")
        ...
        self.assert_true("maybeFinishTelegramOnboarding" in html and "communicationIndex" in html and "setOnboardingFlowStep(communicationIndex" in html, "Auto-selecting the detected Telegram chat advances to the final communication preference")
        self.assert_true("Palabras simples" in html and "Directo, claro y sin jerga." in html and "Explicaciones técnicas" in html and "Más detalle cuando ayude a decidir." in html and "saveCommunicationStyle(event,true)" in html and "/api/onboarding/communication-style" in html, "Onboarding ends with a short global simple-or-technical communication preference")
        self.assert_true("effectiveStyle=saved||(onboarding?'simple':'')" in html and "value=\"simple\" required ${simpleChecked}" in html, "The final onboarding preference defaults to simple words so Save and open dashboard is not a dead click")
