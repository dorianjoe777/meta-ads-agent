...
class IntegrationTestSuite:
    ...
    def test_setup_page_contains_unlock_and_trust(self):
        ...
        self.assert_true("data-input-code=\"autoSaveTelegramToken(event)\"" in html and "telegram-token-zone" in html and "telegram-token-saved-inline" in html and "Clave guardada" in html and "Ahora abre el bot que creaste" in html, "Telegram bot token is saved automatically after paste with a clear saved state and next action")
    ...
    def test_release_package_excludes_runtime_data_and_includes_buyer_docs(self):
        ...
        self.assert_true("META_ADS_AGENT_VERSION=v1.0.11" in env_example and (ROOT_DIR / "VERSION").read_text(encoding="utf-8").strip() == "v1.0.11", "Buyer release exposes the installed product version")
        ...
        self.assert_true("clearCloudInstallation" in portal_digitalocean_api and "buyer_confirmed_deleted_droplet" in portal_digitalocean_api and "cloud_installation: null" in portal_digitalocean_api, "DigitalOcean reset clears the saved cloud install server-side")
