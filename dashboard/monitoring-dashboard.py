...
from daily_agent import approve as approve_pending, reject as reject_pending, run_daily as run_scheduled_daily
from graph_executor import execute_upload_payload
from hermes_bridge import safe_image_paths
from license import activate_license, default_device_id, license_status, normalize_license_entitlements, validate_license_key
...
def activate_license_now(payload=None):
    payload = payload or {}
    status = activate_license(load_config(), transfer_device=bool(payload.get("transfer_device")))
...
def route_chat_action(payload):
    ...
    if wants_pause or wants_resume or wants_budget or wants_creative:
        ...
        if wants_creative:
            ...
            action["reply"] = chat_reply(payload, f"Listo. Preparé ideas de imágenes y textos para {campaign.get('name')}. Las puedes revisar en Imágenes y textos.", f"Done. I prepared image and text ideas for {campaign.get('name')}. You can review them in Images and text.")
            return local_chat_route(payload, action)
    return None
...
async function load(){state=await api('/api/dashboard');render();if(!uiWorkbenchPreview&&state.config.dashboard_password_required&&state.config.dashboard_password_set&&!dashboardPassword()&&state.onboarding&&state.onboarding.completed)showUnlock(t('unlock_needed'));checkForUpdates(false)}
function render(){
 applyTranslations();
 hydrateChatHistory();
...
function renderCreativeStudio(){
 ...
 qs('#creative-library-title').textContent=lang==='es'?'Imágenes y textos para revisar':'Images and text to review';
...
function metaConnectionGuide(){
 const v=state.config.setup_values||{};
 if(lang==='es')return `<div class="setup-guide private-connection"><section class="guide-hero"><div class="guide-main"><span class="guide-eyebrow">Conexión privada</span><h3>Tu propia conexión de Meta</h3><p>Usa las imágenes de guía incluidas con tu compra. La clave nace en tu cuenta de Meta, se guarda en este computador o VPS y la puedes quitar cuando quieras desde Meta.</p><div class="guide-visual"><div class="mini-screen"><span></span><span></span><strong>1. Meta Developers</strong><em>crea tu app</em></div><div class="guide-arrow">&rarr;</div><div class="mini-screen"><span></span><span></span><strong>2. Clave propia</strong><em>cópiala de Meta</em></div><div class="guide-arrow">&rarr;</div><div class="mini-screen"><span></span><span></span><strong>3. Dashboard local</strong><em>pégala aquí</em></div></div><div class="guide-actions"><a class="btn" href="/api/social/login" target="_blank" rel="noopener" onclick="connectMetaStarted()">Abrir Meta</a><button class="btn" type="button" onclick="showMetaTokenBox()">Ya tengo mi clave</button><button class="btn primary" type="button" onclick="refreshSocialAccounts()">Buscar cuentas</button></div><div id="meta-token-box" class="token-box"><label>Clave de acceso de Meta<textarea id="meta-token-input" oninput="scheduleMetaTokenAutoSave()" onpaste="setTimeout(scheduleMetaTokenAutoSave,0)" placeholder="Pega aquí la clave que generaste en tu propia app de Meta"></textarea></label><button class="btn" type="button" onclick="saveMetaToken()">Reintentar guardar</button><p class="notice">Se guarda automáticamente al pegarla. Nosotros no recibimos esta clave; queda local en esta instalación.</p></div></div><aside class="guide-checklist"><b>Sigue tus imágenes de guía</b><ol><li>Crea una app nueva en Meta Developers.</li><li>Abre Marketing API o Graph API Explorer.</li><li>Genera una clave con permisos de anuncios y páginas.</li><li>Pega la clave aquí; el dashboard la guarda solo.</li><li>Busca tus cuentas y elige la correcta.</li></ol></aside></section><div id="social-account-results" class="setup-guide"></div><div class="guide-panel"><b>Por qué esto es más seguro</b><p>La conexión queda entre tu cuenta de Meta y tu instalación local. Si algún día quieres cortar acceso, quitas la clave desde Meta y listo.</p></div></div>`;
 return `<div class="setup-guide private-connection"><section class="guide-hero"><div class="guide-main"><span class="guide-eyebrow">Private connection</span><h3>Your own Meta connection</h3><p>Use the screenshots included with your purchase. The access key starts inside your Meta account, is stored on this computer or VPS, and can be revoked whenever you want.</p><div class="guide-visual"><div class="mini-screen"><span></span><span></span><strong>1. Meta Developers</strong><em>create your app</em></div><div class="guide-arrow">&rarr;</div><div class="mini-screen"><span></span><span></span><strong>2. Your access key</strong><em>copy it from Meta</em></div><div class="guide-arrow">&rarr;</div><div class="mini-screen"><span></span><span></span><strong>3. Local dashboard</strong><em>paste it here</em></div></div><div class="guide-actions"><a class="btn" href="/api/social/login" target="_blank" rel="noopener" onclick="connectMetaStarted()">Open Meta</a><button class="btn" type="button" onclick="showMetaTokenBox()">I have my key</button><button class="btn primary" type="button" onclick="refreshSocialAccounts()">Find accounts</button></div><div id="meta-token-box" class="token-box"><label>Meta access key<textarea id="meta-token-input" oninput="scheduleMetaTokenAutoSave()" onpaste="setTimeout(scheduleMetaTokenAutoSave,0)" placeholder="Paste the key you generated in your own Meta app"></textarea></label><button class="btn" type="button" onclick="saveMetaToken()">Retry save</button><p class="notice">It saves automatically when pasted. We do not receive this key; it stays local to this install.</p></div></div><aside class="guide-checklist"><b>Follow your screenshots</b><ol><li>Create a new app in Meta Developers.</li><li>Open Marketing API or Graph API Explorer.</li><li>Generate a key with ads and Page permissions.</li><li>Paste the key here; the dashboard saves it automatically.</li><li>Find your accounts and choose the right one.</li></ol></aside></section><div id="social-account-results" class="setup-guide"></div><div class="guide-panel"><b>Why this is safer</b><p>The connection stays between your Meta account and your local install. If you ever want to cut access, revoke the key in Meta.</p></div></div>`;
}
...
function renderTelegramPanel(){
 ...
 <div class="field wide"><label>${lang==='es'?'Clave de tu bot de Telegram':'Your Telegram bot key'}</label><span class="field-help">${lang==='es'?'Te la entrega @BotFather cuando creas el bot.':'BotFather gives it to you when you create the bot.'}</span><input type="password" name="bot_token" value="" placeholder="${v.bot_configured?(lang==='es'?'Bot guardado. Pega otro solo si quieres cambiarlo.':'Bot saved. Paste another only to replace it.'):'123456:ABC...'}"></div>
 ...
 </form><div id="telegram-results"></div><p class="notice">${lang==='es'?'Pasos: crea un bot con @BotFather, guarda la clave que te entrega, escríbele cualquier mensaje al bot y toca Detectar mi chat. Cuando el dashboard está encendido, el bot queda escuchando.':'Steps: create a bot with @BotFather, save the key it gives you, send the bot any message, then click Detect my chat. While the dashboard is running, the bot stays listening.'}</p>`;
...
function renderCloudAccessPanel(){
 qs('#cloud-access-panel').innerHTML=`<div class="next-step"><div><b>${lang==='es'?'Abrir acceso cuando estás en la nube':'Cloud dashboard access'}</b><p>${lang==='es'?'Si instalaste en DigitalOcean y cambiaste de Wi-Fi o ciudad, este botón permite que esta red abra tu dashboard.':'If you installed on DigitalOcean and changed Wi-Fi or city, this button allows this network to open your dashboard.'}</p></div><div class="mode-actions"><button class="btn" type="button" onclick="refreshCloudAccess()">${lang==='es'?'Permitir esta red':'Allow this network'}</button></div></div><div id="cloud-access-result"></div><p class="notice">${lang==='es'?'Si ahora no puedes abrir el dashboard en la nube, entra una vez desde DigitalOcean para recuperar acceso y luego usa este botón.':'If you cannot open the cloud dashboard right now, use DigitalOcean recovery access once, then use this button.'}</p>`;
}
