...
from daily_agent import (
    ...
    lead_ads_terms_guidance_from_steps,
    meta_budget_api_amount,
...
RECENT_CREATIVE_RETENTION_DAYS = 3
...
def timezone_preference_state():
    state = read_json(TIMEZONE_PREFERENCE_FILE, {})
    return state if isinstance(state, dict) else {}


def save_timezone_preference(timezone_name, source, account_id=""):
    raw_timezone = str(timezone_name or "").strip()
    normalized = normalize_timezone(raw_timezone, default="") if raw_timezone else ""
    if not normalized or normalized != raw_timezone:
        raise ValueError("No pude reconocer esa zona horaria.")
    state = {
        "timezone": normalized,
        "source": str(source or "").strip().lower() or "buyer",
        "account_id": clean_ad_account_id(account_id) if account_id else "",
        "updated_at": now_iso(),
    }
    write_private_json(TIMEZONE_PREFERENCE_FILE, state, ensure_ascii=False)
    update_env_values({
        "DAILY_BRIEF_TIMEZONE": normalized,
        "DAILY_BRIEF_TIMEZONE_SOURCE": state["source"],
    })
    return state
...
def managed_account_from(value):
    ...
    raw_timezone = str(value.get("timezone_name") or value.get("timezone") or "").strip()
    account_timezone = normalize_timezone(raw_timezone, default="") if raw_timezone else ""
    if account_timezone != raw_timezone:
        account_timezone = ""
    return {
        "id": account_id,
        ...
        "timezone_offset_hours_utc": value.get("timezone_offset_hours_utc"),
...
def save_daily_brief_schedule(payload):
    ...
    update_env_values({"DAILY_BRIEF_TIME": normalized_time})
    save_timezone_preference(normalized_timezone, "buyer")
    ad_config = read_json(AD_CONFIG_FILE, {})
...
def save_daily_social_content_settings(payload):
    ...
    if raw_timezone:
        normalized_timezone = normalize_timezone(raw_timezone, default="")
        if not normalized_timezone or normalized_timezone != raw_timezone:
            raise ValueError("No pude reconocer la zona horaria para los posts diarios.")
        save_timezone_preference(normalized_timezone, "buyer")
    try:
...
def _safe_meta_oauth_summary(connection):
    ...
    return {
        ...
        "accounts": [{key: item.get(key) for key in ("id", "account_id", "name", "currency", "timezone_name", "timezone_offset_hours_utc", "account_status")} for item in connection.get("accounts", []) if isinstance(item, dict)],
...
def _refresh_meta_oauth_pending_once():
    """Apply a completed one-time OAuth handoff from any caller process.

    Hermes MCP tools may run in a short-lived process, so a daemon polling
    thread started while sending the link is not a reliable completion path.
    The buyer's next workspace-status request must be able to finish the
    handoff itself.
    """
    pending = _meta_oauth_pending()
    if not pending.get("request_id") or not pending.get("handoff_secret"):
        return None
    result = _meta_oauth_request(
        str(pending.get("broker_url") or ""),
        {
            "flow": "poll",
            "request_id": pending["request_id"],
            "handoff_secret": pending["handoff_secret"],
        },
    )
    if result.get("ok") and result.get("status") == "connected":
        summary = _apply_meta_oauth_credentials(result.get("credentials"))
        try:
            META_OAUTH_PENDING_FILE.unlink()
        except OSError:
            pass
        log_action(
        ...
        return summary
    if result.get("error") in {"oauth_request_expired", "oauth_callback_failed"}:
        try:
            META_OAUTH_PENDING_FILE.unlink()
        except OSError:
            pass
...
def social_oauth_status():
    config = load_config()
    try:
        _refresh_meta_oauth_pending_once()
    except Exception:
        # Status remains safe and readable during a transient broker/network
        # failure. A later buyer message can retry the same pending handoff.
        pass
    connection = _meta_oauth_connection()
    pending = _meta_oauth_pending()
...
def live_ad_account_timezone_metadata(account):
    account = dict(account) if isinstance(account, dict) else {"id": account}
    account_id = clean_ad_account_id(account.get("id") or account.get("account_id"))
    if not account_id:
        return account
    result = graph_get(
        f"/{account_id}",
        {"fields": "id,account_id,name,currency,timezone_name,timezone_offset_hours_utc,account_status,business{id,name}"},
    )
    live = result.get("data") if result.get("ok") and isinstance(result.get("data"), dict) else {}
    return {**account, **{key: value for key, value in live.items() if value not in (None, "")}}


def synchronize_selected_ad_account_timezone(account=None, reconcile_crons=False):
    """Use the selected Meta account timezone unless the buyer overrode it."""
    account_id = clean_ad_account_id((account or {}).get("id") if isinstance(account, dict) else account)
    account_id = account_id or current_configured_ad_account_id()
    if not account_id:
        return {"ok": False, "reason": "missing_ad_account"}
    account = live_ad_account_timezone_metadata(account or {"id": account_id})
    raw_timezone = str(account.get("timezone_name") or "").strip()
    timezone_name = normalize_timezone(raw_timezone, default="") if raw_timezone else ""
    if not timezone_name or timezone_name != raw_timezone:
        return {"ok": False, "reason": "missing_meta_account_timezone", "account_id": account_id, "account": account}
    existing = timezone_preference_state()
    if str(existing.get("source") or "").strip().lower() == "buyer" and existing.get("timezone"):
        return {
            "ok": True,
            "changed": False,
            "source": "buyer",
            "timezone": existing["timezone"],
            "meta_timezone": timezone_name,
            "account_id": account_id,
            "account": account,
        }
    changed = existing.get("timezone") != timezone_name or existing.get("account_id") != account_id or existing.get("source") != "meta_ad_account"
    state = save_timezone_preference(timezone_name, "meta_ad_account", account_id=account_id)
    cron_result = reconcile_timezone_crons(load_config()) if reconcile_crons and changed else {"reconciled": False, "needed": False}
...
def social_oauth_start(payload=None):
    ...
    # Idempotency guard: a model may call the setup tool again after a
    # context compaction or campaign request. Never create a second OAuth
    # handoff while a valid connection is already persisted.
    connected = _meta_oauth_connection()
    if bool(connected.get("connected")):
        return {
            "ok": True,
            "connected": True,
            "already_connected": True,
            "sent_to_telegram": False,
            **_safe_meta_oauth_summary(connected),
        }
    ...
    # daemon thread here would disappear as soon as the current turn exits.
...
def campaign_location_query_terms(value):
    ...
    for item in values:
        if isinstance(item, dict):
            ...
            continue
        if isinstance(item, str) and re.search(r",|;|/|\||\by\b|\band\b", item, flags=re.IGNORECASE):
            # Terra commonly emits a JSON list whose single item is the
            # buyer's natural "City, Country" phrase. Parse that item with
            # the same logic as a scalar instead of letting the country name
            # cause the whole combined value to be classified as a country.
            for nested in campaign_location_query_terms(item):
                identity = _plain_location_text(nested)
...
def graph_marketing_accounts():
    direct = graph_get(
        "/me/adaccounts",
        {"fields": "id,account_id,name,currency,timezone_name,timezone_offset_hours_utc,account_status,business{id,name}", "limit": 100},
    )
    if direct.get("ok"):
    ...
    businesses = graph_get(
        "/me/businesses",
        {
            "fields": "id,name,owned_ad_accounts{id,account_id,name,currency,timezone_name,timezone_offset_hours_utc,account_status},client_ad_accounts{id,account_id,name,currency,timezone_name,timezone_offset_hours_utc,account_status}",
            "limit": 50,
        },
    )
...
def social_set_default_account(payload):
    ...
    if timezone_sync.get("changed"):
        saved["timezone_crons"] = reconcile_timezone_crons(load_config())
    return {
        "ok": True,
        "code": 0,
...
def hermes_browserless_command(config):
    # Use Codex' native device flow as the single source of truth. It writes a
    # complete CLI credential (including id_token) under this buyer's isolated
    # CODEX_HOME; after login we import that session into Hermes. Starting with
    # Hermes auth here created two independent stores and allowed the dashboard
    # to show a newly connected account while Image/Codex CLI kept the old one.
    codex_cli = shutil.which(str(getattr(config, "codex_cli", "") or "codex").strip() or "codex")
    if codex_cli:
        return [codex_cli, "login", "--device-auth"]
    cli = str(getattr(config, "hermes_cli", "") or "hermes").strip() or "hermes"
    if hermes_explicit_codex_auth_supported(config):
        return [cli, "auth", "add", "openai-codex", "--no-browser", "--timeout", "300"]
...
def browserless_chatgpt_ready(config):
    """Treat a stored Codex subscription as connected even with Gemini primary.

    ``hermes_codex_ready`` intentionally requires Codex to be the active text
    provider.  This browserless flow connects the shared subscription used by
    Image 2 and the Terra fallback, so a positive Codex authentication signal
    is sufficient while Google AI Studio remains the primary brain.
    """
    status = hermes_codex_session_status(
        config,
        timeout=runtime_codex_status_timeout(config),
    )
    authenticated = bool(status.get("authenticated")) and not bool(status.get("reauth_required"))
    return authenticated, str(status.get("detail") or "OpenAI Codex auth unknown")


def hermes_browserless_launch_command(config):
    """Resolve the executable selected by ``hermes_browserless_command``.

    The native Codex device flow deliberately uses the Codex CLI, while the
    compatibility paths use Hermes.  Resolve that selected binary instead of
    replacing it with ``hermes`` immediately before launch.
    """
    command = list(hermes_browserless_command(config))
    if not command:
        return []
    selected = str(command[0] or "").strip()
    candidate = Path(selected).expanduser()
    executable = selected if candidate.is_absolute() else shutil.which(selected)
    if not executable:
...
def refresh_agent_runtime_status(payload=None):
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=5, thread_name_prefix="admira-runtime") as executor:
            ...
            image_status_future = executor.submit(
                hermes_codex_image_status,
                timeout=max(8, min(15, status_timeout)),
                config=config,
            )
            ...
            image_status = image_status_future.result()
            image_session = image_future.result() if image_future else main_session
            if image_status.get("ok"):
                # The text brain can be Gemini while the shared ChatGPT
                # subscription independently powers openai-codex Image. Do
                # not copy Gemini's Codex-session status into the Image card.
                image_session = {
                    "ready": True,
                    "authenticated": True,
                    "provider_ready": True,
                    "reauth_required": False,
                    "auth_state": "stored",
                    "detail": image_status.get("detail") or "ChatGPT/Codex listo para imágenes",
                    "identity": (image_session or {}).get("identity") or {},
                    "returncode": 0,
                }
            result = {
                "main_codex_session": main_session,
                "image_codex_session": image_session,
                "codex_image_status": image_status,
...
def codex_cli_auth_fingerprint(config):
    """Hash only the canonical Codex CLI credential used by device login."""
    environment = hermes_environment(config)
    ...
    digest = hashlib.sha256()
    found = False
    for relative in ("auth.json", ".codex/auth.json", "credentials.json"):
        path = home / relative
        try:
...
def hermes_browserless_snapshot(config=None, purpose="agent"):
    ...
    replacement_ready = bool(ready and (not force_reconnect or replacement_changed))
...
def import_codex_cli_session_into_hermes(config):
    """Import the just-completed native Codex login into the matching Hermes home."""
    environment = hermes_environment(config)
    script = (
        "from hermes_cli.auth import _import_codex_cli_tokens as load, _save_codex_tokens as save; "
        "tokens=load(); assert tokens, 'Codex CLI login did not produce credentials'; save(tokens)"
    )
    try:
        result = subprocess.run(
...
def finish_hermes_browserless_session(session_id, returncode):
    ...
    config = config_for_connect_purpose(purpose)
    if returncode == 0:
        import_codex_cli_session_into_hermes(config)
    ready, auth_detail = browserless_chatgpt_ready(config)
    with HERMES_LOGIN_LOCK:
        force_reconnect = bool(HERMES_LOGIN_STATE.get("force_reconnect"))
        baseline_fingerprint = str(HERMES_LOGIN_STATE.get("baseline_auth_fingerprint") or "")
    replacement_changed = codex_cli_auth_fingerprint(config) != baseline_fingerprint
    ready = bool(ready and (not force_reconnect or replacement_changed))
    ...
    if ready and purpose == "agent":
        ...
        try:
            ...
            pass
        # The long-lived Telegram gateway may still hold the previous OAuth
        # provider instance. Rebuild it only after the new credential is fully
        # persisted so the next buyer message uses the selected account.
        try:
            refresh_telegram_gateway_after_agent_model_change(
                {"HERMES_MODEL": getattr(config, "hermes_model", "")},
                force_restart=True,
            )
        except Exception:
...
def start_hermes_browserless_login(config, purpose="agent", force_reconnect=False):
    purpose = normalize_connect_purpose(purpose)
    command = hermes_browserless_launch_command(config)
    if not command:
        return hermes_connect_response(
            "not_installed",
    ...
    try:
        ...
        baseline_auth_fingerprint = codex_cli_auth_fingerprint(config)
        master_fd, slave_fd = pty.openpty()
        explicit_auth = (
            ("auth" in command and "openai-codex" in command)
            or ("login" in command and "--device-auth" in command)
        )
        env = hermes_environment(config)
            ...
            HERMES_LOGIN_STATE.update(
                {
                    ...
                    "baseline_auth_fingerprint": baseline_auth_fingerprint,
                }
            )
...
def connect_agent_model(payload=None):
    ...
    if purpose == "image":
        ...
        return start_hermes_browserless_login(
            config,
            purpose="image",
            force_reconnect=bool(payload.get("force_reconnect")),
        )
        ...
        managed_state = prepare_managed_ad_account_update(
            {
                ...
                "timezone_name": payload.get("account_timezone", ""),
                "timezone_offset_hours_utc": payload.get("account_timezone_offset"),
...
def recent_generated_creatives(when="last_3_days", limit=24, cleanup=True):
    """List short-lived generated images by human dates and prune expired files.

    Durable content-library copies and media referenced by pending approvals are
    excluded from cleanup. This is a lightweight Telegram recovery window, not
    a permanent DAM/library.
    """
    now = datetime.now(timezone.utc)
    cutoff = now - timedelta(days=RECENT_CREATIVE_RETENTION_DAYS)
    protected_text = ""
    for state_path in (CONTENT_ASSET_LIBRARY_FILE, PENDING_FILE, ORGANIC_CONTENT_POSTS_FILE):
        try:
            if state_path.exists():
                protected_text += state_path.read_text(encoding="utf-8")
        except OSError:
            continue
    removed = []
    candidates = []
    if CREATIVE_ASSET_ROOT.exists():
        for path in CREATIVE_ASSET_ROOT.glob("codex-*/*"):
            if not path.is_file() or path.suffix.lower() not in CREATIVE_ASSET_EXTENSIONS:
                continue
            try:
                ...
                continue
            if cleanup and created_at < cutoff and resolved not in protected_text:
                try:
                    path.unlink()
            ...
            if created_at >= cutoff:
                candidates.append((created_at, path, resolved))
        if cleanup:
            for directory in sorted(CREATIVE_ASSET_ROOT.glob("codex-*"), reverse=True):
                try:
                    if directory.is_dir() and not any(directory.iterdir()):
                        directory.rmdir()
                except OSError:
                    continue
    buyer_tz = automatic_update_timezone(load_config())
    local_today = now.astimezone(buyer_tz).date()
    normalized_when = str(when or "last_3_days").strip().lower().replace(" ", "_")
    items = []
    for created_at, path, resolved in sorted(candidates, key=lambda row: row[0], reverse=True):
        local_date = created_at.astimezone(buyer_tz).date()
        if normalized_when in {"today", "hoy"} and local_date != local_today:
            continue
        if normalized_when in {"yesterday", "ayer"} and local_date != local_today - timedelta(days=1):
            continue
        relative = "today" if local_date == local_today else "yesterday" if local_date == local_today - timedelta(days=1) else f"{(local_today - local_date).days}_days_ago"
        items.append({
            "asset_id": str(path.resolve().relative_to(CREATIVE_ASSET_ROOT.resolve())),
            "file_path": resolved,
        ...
        if len(items) >= max(1, min(50, int(limit or 24))):
            break
    return {
        "ok": True,
        "retention_days": RECENT_CREATIVE_RETENTION_DAYS,
        "when": normalized_when,
        "timezone": str(buyer_tz),
        "items": items,
...
def agent_onboarding_phase(profile=None):
    ...
    facebook_authorized = bool(oauth.get("connected"))
    facebook_connected = facebook_authorized and bool(oauth.get("active_ad_account_id")) and bool(oauth.get("active_page_id"))
    workspace_selection_required = facebook_authorized and not facebook_connected
    if not facebook_authorized:
    ...
    elif workspace_selection_required:
        phase = "facebook_connection"
        next_step = (
    ...
    return {
        ...
        "oauth_account_count": len(oauth.get("accounts") or []),
...
def save_agent_preferences(payload, restart_gateway=True):
    ...
    if raw_timezone and timezone_name != raw_timezone:
        raise ValueError("No pude reconocer esa zona horaria.")
    if not style and not ad_experience and not timezone_name:
    ...
    update_env_values(updates)
    if timezone_name:
        save_timezone_preference(timezone_name, "buyer")
    ...
    log_action("agent_preferences_update", {"communication_style": style, "ad_experience_level": ad_experience, "timezone": timezone_name}, "completed")
    return {
        "saved": True,
        "ad_experience_level": ad_experience,
        "timezone": timezone_name or load_config().daily_brief_timezone,
...
def execute_approved_account_action(config, action_type, campaign, action_payload):
    ...
    if action_type == "pause_campaign":
        ...
        currency = str(action_payload.get("account_currency") or configured_ad_account_currency() or "").strip().upper()
        api_amount = meta_budget_api_amount(action_payload.get("new_budget", 0), currency)
        result = client.set_budget(target_type, target_id, api_amount)
        action_payload["meta_budget_api_amount"] = api_amount
        action_payload["account_currency"] = currency
    else:
...
def apply_action(payload):
    ...
    if action in {"adjust_budget", "apply_recommendation"}:
        ...
        account_currency = configured_ad_account_currency()
        budget_confirmation = str(payload.get("budget_confirmation") or f"{new_budget:g} {account_currency}").strip()
        budget_contract = confirmed_budget_contract(budget_confirmation, account_currency=account_currency)
        if not budget_contract.get("ok"):
            raise ValueError(
                "No preparé el cambio de presupuesto: el monto y la moneda deben coincidir exactamente "
                f"con la cuenta publicitaria ({account_currency or 'moneda desconocida'})."
            )
        new_budget = float(budget_contract["amount"])
        change_pct = abs((new_budget - current) / current * 100) if current else 100
        action_payload = {
            "campaign_id": campaign_id,
            "name": campaign.get("name"),
            "target_type": campaign.get("target_type", "campaign"),
            "target_id": campaign.get("target_id", campaign_id),
            "current_budget": current,
            "new_budget": new_budget,
            "budget_confirmation": budget_confirmation,
            "account_currency": budget_contract["currency"],
            "meta_budget_api_amount": budget_contract["api_amount"],
            "change_pct": round(change_pct, 1),
        }
...
def verified_campaign_stack_execution(result):
    """A campaign exists only after Meta returns IDs for all three levels."""
    if not isinstance(result, dict) or result.get("executed") is not True:
        return False
    campaign_id = str(result.get("campaign_id") or "").strip()
    adset_ids = [str(value).strip() for value in (result.get("adset_ids") or []) if str(value).strip()]
    ad_ids = [str(value).strip() for value in (result.get("ad_ids") or []) if str(value).strip()]
...
def enrich_campaign_location_selections(items, search=None):
    """Complete abbreviated model locations from Meta before any mutation."""
    search = search or meta_targeting_search
    enriched = []
    for original in items or []:
        ...
        if not isinstance(item, dict):
            ...
            continue
        location_type = str(item.get("type") or "").strip().lower()
        if location_type in {"country", "city", "region", "zip", "geo_market"}:
            enriched.append(item)
            continue
        key = str(item.get("key") or item.get("id") or "").strip()
        query = str(item.get("name") or item.get("label") or key).strip()
        result = search({"kind": "location", "q": query, "limit": 25}) if query else {}
        candidates = result.get("items") or [] if isinstance(result, dict) and result.get("ok") else []
        exact = [candidate for candidate in candidates if str(candidate.get("key") or candidate.get("id") or "").strip() == key]
        if len(exact) == 1:
            item = {**item, **exact[0]}
        enriched.append(item)
    return enriched
...
def create_campaign(payload):
    ...
    selected_locations = parse_targeting_items(campaign_location_selection_source(payload), "location")
    if selected_locations:
        selected_locations = enrich_campaign_location_selections(selected_locations)
        incomplete_location = next((
            item for item in selected_locations
            if isinstance(item, dict)
            and str(item.get("type") or "").strip().lower() not in {"country", "city", "region", "zip", "geo_market"}
        ), None)
        if incomplete_location:
            raise ValueError(
                "targeting_location_incomplete: Meta no pudo confirmar el tipo de la ubicación estructurada. "
                "No se creó ningún objeto."
            )
    ...
    if explicit_ad_sets:
        inherited_ad = {
            ...
            "cta": str(payload.get("cta") or ("" if payload.get("on_meta_destination") else "LEARN_MORE")).strip().upper(),
            ...
            "on_meta_destination": bool(payload.get("on_meta_destination")),
        ...
        for index, raw_set in enumerate(explicit_ad_sets):
            ...
            structured_nested_locations = []
            for item in (
                nested_location_source
                if isinstance(nested_location_source, list)
                else [nested_location_source]
            ):
                if not isinstance(item, dict):
                    continue
                key = str(item.get("key") or item.get("id") or "").strip()
                location_type = str(item.get("type") or item.get("location_type") or "").strip().lower()
                if key and location_type in {"country", "city", "region"}:
                    structured_nested_locations.append(dict(item))
            ...
            if structured_nested_locations:
                # A city/region key selected from Meta is more specific than
                # the accompanying ISO country code.  The latter is useful
                # lookup metadata, not a second audience instruction.
                raw_targeting = dict(raw_targeting)
                raw_targeting["locations"] = structured_nested_locations
                for broad_key in ("countries", "targeting_countries", "country_codes"):
                    raw_targeting.pop(broad_key, None)
                raw_meta_targeting = raw_targeting.get("meta_targeting") if isinstance(raw_targeting.get("meta_targeting"), dict) else {}
                raw_targeting["meta_targeting"] = {**raw_meta_targeting, "locations": structured_nested_locations}
            elif nested_location_queries:
            ...
            for key in ("placements", "billing_event", "bidding", "start_time", "end_time", "is_adset_budget_sharing_enabled", "optimization_goal", "promoted_object", "destination_type", "message_destination", "whatsapp_phone_number_id"):
                if raw_set.get(key) not in (None, ""):
                    nested_set[key] = (
                        normalize_placement_config(raw_set[key])
                        if key == "placements"
                        else raw_set[key]
                    )
        ...
        if execution_result.get("ok") and execution_result.get("executed") and not succeeded:
            execution_result = {
                **execution_result,
                "ok": False,
                "blocked": True,
                "reason": "campaign_creation_missing_real_ids",
                "message": "Meta did not confirm campaign, ad-set, and ad IDs; creation cannot be reported as successful.",
            }
        status = "created_paused" if succeeded else "failed"
...
def normalize_campaign_ad_sets(arguments):
    ...
    for index, raw in enumerate(raw_sets):
        ...
        if entry.get("placements") not in (None, "", [], ()):
            entry["placements"] = normalize_placement_config(entry["placements"])
...
def confirmed_budget_contract(value, account_currency=""):
    """Resolve one buyer-facing budget quote against the live account currency."""
    raw = str(value or "").strip()
    amount = parse_money_like(raw, default=None)
    currency = str(account_currency or configured_ad_account_currency() or "").strip().upper()
    if amount is None or amount <= 0:
        return {"ok": False, "reason": "invalid_budget_confirmation", "raw": raw}
    if not currency:
        return {"ok": False, "reason": "missing_ad_account_currency", "raw": raw, "amount": amount}
    mentioned_currency = currency_hint_from_text(raw)
    # A bare currency symbol or generic peso wording is intentionally resolved
    # from the selected ad account. Explicit ISO/named currencies must match.
    ambiguous_account_currency = any(token in raw.upper() for token in ("$", "PESO", "PESOS"))
    if not mentioned_currency and not ambiguous_account_currency:
        return {
            "ok": False,
            "reason": "missing_budget_currency",
            "raw": raw,
            "amount": amount,
            "account_currency": currency,
        }
    if mentioned_currency and mentioned_currency != currency:
        return {
            "ok": False,
            "reason": "budget_currency_mismatch",
            "raw": raw,
            "amount": amount,
            "mentioned_currency": mentioned_currency,
            "account_currency": currency,
        }
    api_amount = meta_budget_api_amount(amount, currency)
    if api_amount <= 0:
        return {
            "ok": False,
            "reason": "unsupported_ad_account_currency",
            "raw": raw,
            "amount": amount,
            "account_currency": currency,
        }
    return {
        ...
        "amount": float(amount),
        "currency": currency,
        "mentioned_currency": mentioned_currency or currency,
...
def normalize_campaign_stack_arguments(arguments, chat_payload=None):
    ...
    if normalized_ad_sets:
        # A flat ads[] payload is materialized as an implicit ad set. Carry
        # the destination-level audience contract into that set as well. The
        # campaign payload is normalized more than once before execution; if
        # an implicit set contains only placements on the second pass,
        # create_campaign() correctly treats it as explicit but would
        # otherwise lose the verified city, ages and genders.
        geo_aliases = ("locations", "targeting_locations", "targeting_countries", "countries", "country_codes")
        targeting_groups = (
            ("age_range", "age_min", "age_max", "min_age", "max_age"),
            ("genders", "gender", "targeting_gender", "targeting_genders", "sex"),
            ("interests", "targeting_interests", "meta_targeting"),
            ("targeting_mode", "targeting_automation"),
        )
        for item in normalized_ad_sets:
            nested_targeting = dict(item.get("targeting") or {}) if isinstance(item.get("targeting"), dict) else {}
            if not any(nested_targeting.get(key) not in (None, "", [], (), {}) for key in geo_aliases):
                # Prefer a verified Meta selection over the derived country
                # fallback. Keeping both makes later code choose `locations`
                # first and discards the exact city key.
                if args.get("targeting_locations") not in (None, "", [], (), {}):
                    value = args["targeting_locations"]
                    nested_targeting["targeting_locations"] = list(value) if isinstance(value, list) else dict(value) if isinstance(value, dict) else value
                else:
                    for key in ("locations", "targeting_countries", "countries", "country_codes"):
                        value = args.get(key)
                        if value not in (None, "", [], (), {}):
                            nested_targeting[key] = list(value) if isinstance(value, list) else value
                            break
            # A first normalization may already have derived the broad
            # country code (for example CO) while retaining the buyer's exact
            # natural city as a live-lookup query.  Propagate that query even
            # when the nested set now contains the derived country; otherwise
            # a second normalization silently broadens Cartagena to Colombia.
            if (
                nested_targeting.get("targeting_location_queries") in (None, "", [], (), {})
                and args.get("targeting_location_queries") not in (None, "", [], (), {})
            ):
                nested_targeting["targeting_location_queries"] = list(args["targeting_location_queries"])
            for aliases in targeting_groups:
                if any(nested_targeting.get(key) not in (None, "", [], (), {}) for key in aliases):
                    continue
                for key in aliases:
                    value = args.get(key)
                    if value in (None, "", [], (), {}):
                        continue
                    if isinstance(value, dict):
                        nested_targeting[key] = dict(value)
                    elif isinstance(value, list):
                        nested_targeting[key] = list(value)
                    else:
                        nested_targeting[key] = value
            if nested_targeting:
                item["targeting"] = nested_targeting
        # Destination MCPs ask for one explicit placement decision at the
        # campaign level. Flat ads are then grouped into an implicit ad set;
        # carry that decision into every set that did not override it. Losing
        # it here used to turn {automatic: true} into the legacy manual
        # feed/story default after an otherwise correct Hermes call.
        top_level_placements = args.get("placements")
        if top_level_placements not in (None, "", [], ()):
            for item in normalized_ad_sets:
                if item.get("placements") in (None, "", [], ()):
                    if isinstance(top_level_placements, dict):
                        item["placements"] = dict(top_level_placements)
                    elif isinstance(top_level_placements, list):
                        item["placements"] = list(top_level_placements)
                    else:
                        item["placements"] = top_level_placements
...
def find_pending_approval_for_text(text, pending):
    ...
    if not pending:
        return None, "empty"
    normalized = normalize_text(text)
    # Approval IDs use both historical ``approval_...`` and campaign-edit
    # ``approval-edit-...`` forms. Exact tool-selected IDs must win over the
    # newest pending card when several campaigns are being edited at once.
    id_matches = re.findall(r"approval(?:_|-)[a-zA-Z0-9_\-]+", str(text or ""))
...
def campaign_budget_action(campaign, new_budget, payload, tool_type="adjust_budget", budget_confirmation=""):
    result = apply_action({
    ...
    staged = isinstance(result, dict) and result.get("status") == "pending"
    currency = str((result or {}).get("payload", {}).get("account_currency") if isinstance(result, dict) else "") or configured_ad_account_currency()
    return agent_action_result(
        tool_type,
        not staged,
        chat_reply(payload, f"{'Dejé en aprobación' if staged else 'Ajusté'} el presupuesto de {campaign.get('name')} a {new_budget:,.2f} {currency}.", f"I {'staged for approval' if staged else 'adjusted'} {campaign.get('name')} to {new_budget:,.2f} {currency} daily budget."),
        staged=staged,
        campaign_id=campaign.get("id"),
        new_budget=new_budget,
        result=result,
...
def handle_create_campaign_stack_tool(arguments, chat_payload, tool):
    ...
    has_app_destination = bool(arguments.get("app_destination") or (arguments.get("application_id") and arguments.get("object_store_url")))
    has_on_meta_destination = bool(arguments.get("on_meta_destination"))
    ...
    if not arguments.get("object_story_id") and not has_message_destination and not has_lead_form and not has_app_destination and not has_on_meta_destination:
    ...
    if missing:
        return agent_action_result(
            ...
            chat_reply(
                chat_payload,
                f"No creé nada en Meta porque falta esto: {', '.join(missing)}. Dame ese dato y crearé la campaña directamente en pausa, sin pedir otra aprobación.",
                f"I did not create anything in Meta because this is missing: {', '.join(missing)}. Send that detail and I will create the campaign directly as paused, without another approval.",
            ),
            blocked=True,
...
def normalize_lead_form_tool_arguments(arguments, chat_payload):
    ...
    # Buyers commonly paste a public domain without a scheme in chat. Treat
    # that as an HTTPS URL instead of making the model ask for the same policy
    # link again. Never guess for whitespace, local paths, or non-host text.
    if privacy_url and not privacy_url.startswith(("http://", "https://")):
        host_candidate = privacy_url.split("/", 1)[0]
        if "." in host_candidate and " " not in privacy_url:
            privacy_url = f"https://{privacy_url.lstrip('/')}"
...
def create_lead_form_creation(arguments, chat_payload):
        ...
        if str(form.get("name") or "").strip().casefold() == requested_name:
            ...
            if form_id:
                return agent_action_result(
                    ...
                    ok=True, created=False, reused=True, verified_live=True,
    ...
    if created.get("returncode") != 0:
        terms_guidance = lead_ads_terms_guidance_from_steps([{"step": "create_lead_form", "result": created}])
        if terms_guidance:
            return agent_action_result(
                "create_lead_form", False,
                chat_reply(chat_payload, terms_guidance["message_es"], terms_guidance["message_en"]),
                blocked=True, reason="meta_lead_ads_terms_required",
                buyer_action_required=terms_guidance,
                result=summarize_cli_result(created),
            )
        return agent_action_result(
            "create_lead_form", False,
            chat_reply(
    ...
    return agent_action_result(
        ...
        ok=True, created=True, verified_live=True, lead_gen_form_id=created_id,
        page_id=payload["page_id"], form=verified,
    )


def handle_create_lead_form_tool(arguments, chat_payload, tool):
    """Create a native form and require a successful live read-back.

    The implementation is idempotent by exact form name. Real Meta permission,
    capability, or Lead Ads Terms errors are returned to Hermes; only then may
    the agent offer ``stage_lead_form`` as the manual fallback.
    """
...
def handle_campaign_mutation_tool(arguments, chat_payload, tool):
    ...
    if tool == "set_budget":
        ...
        return campaign_budget_action(
            campaign,
            new_budget,
            chat_payload,
            tool,
            budget_confirmation=str(arguments.get("budget_confirmation") or "").strip(),
        )
...
def handle_campaign_edit_tool(arguments, chat_payload, tool="edit_campaign"):
    """Resolve one natural campaign reference and stage an isolated edit draft."""
    config = load_config()
    live = refresh_managed_real_metrics(
        reason="agent_campaign_edit",
        persist=False,
    ...
    if not live.get("ok"):
        return agent_action_result(
            ...
            chat_reply(
                chat_payload,
                "No pude confirmar el inventario actual de Meta, así que no preparé ningún cambio.",
                "I could not confirm Meta's current campaign inventory, so I prepared no change.",
            ),
            blocked=True,
            reason="live_campaign_inventory_unavailable",
            result=live,
        )
    metrics = dict(live.get("metrics") or {})
    metrics["account_currency"] = configured_ad_account_currency()
    account_id = str(live.get("account_id") or metrics.get("account_id") or "").strip()
    conversation_id = str(getattr(config, "telegram_chat_id", "") or "").strip()
    prepared = prepare_campaign_edit(
        arguments,
        metrics,
        account_id=account_id,
        conversation_id=conversation_id,
        budget_contract=confirmed_budget_contract,
    )
    if not prepared.get("ok"):
        return agent_action_result(
            tool,
            False,
            chat_reply(chat_payload, prepared.get("reply") or "No preparé ese cambio.", prepared.get("reply") or "I did not prepare that change."),
            blocked=True,
            reason=prepared.get("reason") or "campaign_edit_not_ready",
            candidates=prepared.get("candidates") or [],
            missing_fields=prepared.get("missing_fields") or [],
            campaign_id=prepared.get("campaign_id") or "",
            result=prepared,
        )
    pending_payload = dict(prepared.get("pending_payload") or {})
    edit_id = str(prepared.get("edit_id") or "").strip()
    supersede_pending_edit(edit_id)
    approval_id = f"approval-edit-{edit_id}-r{pending_payload.get('revision', 1)}"
    pending_payload["approval_id"] = approval_id
    pending = add_pending("campaign_edit", pending_payload)
    mark_draft_status(pending_payload.get("draft_path"), "pending_approval", result={"approval_id": approval_id, "pending": pending})
    reply = (
        f"Preparé los cambios para {prepared.get('campaign_name')}. {prepared.get('summary') or 'Quedaron listos para revisión.'} "
        "No modifiqué Meta todavía; este cambio quedó separado de cualquier otra campaña y espera aprobación explícita."
    )
    return agent_action_result(
        tool,
        False,
        chat_reply(chat_payload, reply, reply),
        staged=True,
        blocked=False,
        reason="campaign_edit_pending_approval",
        campaign_id=prepared.get("campaign_id"),
        campaign_name=prepared.get("campaign_name"),
        edit_id=edit_id,
        approval_id=approval_id,
        pending=pending,
...
def handle_connect_chatgpt_tool(arguments, chat_payload, tool="connect_chatgpt"):
    """Start the dashboard-owned device OAuth flow without shell guidance."""
    from admira_hermes_runtime_patch import (
        _automatic_codex_recovery,
        _chatgpt_connection_reply,
        _remember_chatgpt_login_pending,
    )

    recovery = _automatic_codex_recovery(wait_seconds=15, action="switch")
    language = str((chat_payload or {}).get("language") or "es")
    reply = _chatgpt_connection_reply(recovery, language)
    login_url = str(recovery.get("url") or "").strip()
    login_code = str(recovery.get("code") or "").strip()
    if login_url and login_code:
        config = load_config()
        chat_id = str(getattr(config, "telegram_chat_id", "") or "").strip()
        pending_key = f"agent:main:telegram:dm:{chat_id}" if chat_id else "default"
        ...
        return agent_action_result(
            ...
            reason="chatgpt_device_login_started",
        )
    return agent_action_result(
        tool,
        False,
        ...
        connected=False,
        needs_login=True,
        reason="chatgpt_device_login_unavailable",
        device_auth_settings=bool(recovery.get("device_auth_settings")),
    )
...
AGENT_TOOL_HANDLERS = {
    ...
    "edit_campaign": handle_campaign_edit_tool,
    "connect_chatgpt": handle_connect_chatgpt_tool,
    ...
    return {
        ...
        "config": {
            ...
            "agent_model": {
                ...
                "brain_provider": getattr(config, "agent_brain_provider", "gemini"),
                ...
                "primary_brain": getattr(config, "agent_brain_provider", "gemini"),
                "codex_image_source": codex_image_source,
                "codex_image_source_configured": getattr(config, "codex_image_source", "main_chatgpt"),
                "codex_image_hermes_model": image_model,
                "codex_image_ready": codex_image_ready,
            ...
            "setup_values": {
                ...
                "agent_brain_provider": getattr(config, "agent_brain_provider", "gemini"),
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def post_internal_model_recovery(self, payload):
        ...
        config = load_config()
        if action == "switch":
            result = reconnect_shared_chatgpt_subscription()
        elif action == "start":
            result = connect_agent_model({"hermes_model": getattr(config, "hermes_model", "")})
        else:
            result = agent_model_connect_status({"connection_purpose": "agent"})
        safe = {
            key: result.get(key)
            for key in (
                "status",
...
def main():
    ...
    try:
        timezone_sync = synchronize_selected_ad_account_timezone(reconcile_crons=False)
    except Exception:
        timezone_sync = {"ok": False, "changed": False}
    config = migrate_nvidia_primary_model(load_config())
