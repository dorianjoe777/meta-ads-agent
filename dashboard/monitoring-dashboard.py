...
function metaConnectionGuide(){
 const v=state.config.setup_values||{};
 if(lang==='es')return `<div class="setup-guide private-connection"><section class="guide-hero"><div class="guide-main"><span class="guide-eyebrow">Paso seguro</span><h3>Conectar mi cuenta de Facebook</h3><p>El camino recomendado es crear una conexión estable en tu propio Meta Business con un <strong>Usuario del sistema</strong>. Así el acceso queda bajo tu control y no depende de un token temporal de prueba.</p><div class="guide-visual"><div class="mini-screen"><span></span><span></span><strong>1. Meta Business</strong><em>usuario del sistema</em></div><div class="guide-arrow">&rarr;</div><div class="mini-screen"><span></span><span></span><strong>2. Token estable</strong><em>cuenta y página</em></div><div class="guide-arrow">&rarr;</div><div class="mini-screen"><span></span><span></span><strong>3. Admiro local</strong><em>pégalo aquí</em></div></div><div class="guide-support-grid"><div class="guide-card"><b>Recomendado: conexión estable</b><p>Para trabajar todos los días. Crea un Usuario del sistema, asígnale tu cuenta publicitaria y página, genera la clave y pégala aquí.</p><div class="onboarding-step-actions"><a class="btn primary" href="/api/social/login" target="_blank" rel="noopener" onclick="connectMetaStarted()">Abrir Meta Business</a><button class="btn" type="button" onclick="showMetaTokenBox()">Ya tengo la clave estable</button></div></div><div class="guide-card"><b>Prueba rápida: token temporal</b><p>Solo para probar. Graph API Explorer puede vencer y pedir reconexión. No es lo ideal para dejar el agente trabajando.</p><div class="onboarding-step-actions"><a class="btn" href="https://developers.facebook.com/tools/explorer/" target="_blank" rel="noopener">Abrir Graph API Explorer</a></div></div></div><div class="guide-actions"><button class="btn primary" type="button" onclick="refreshSocialAccounts()">Buscar mis cuentas</button><button class="btn" type="button" onclick="openChat('Ayúdame a conectar Meta con una clave estable de Usuario del sistema. Explícamelo paso a paso en palabras simples.')">${t('ask_agent')}</button></div><div id="meta-token-box" class="token-box"><label>Clave estable de Facebook/Meta<textarea id="meta-token-input" oninput="scheduleMetaTokenAutoSave()" onpaste="setTimeout(scheduleMetaTokenAutoSave,0)" placeholder="Pega aquí la clave estable que generaste en Meta Business"></textarea></label><button class="btn" type="button" onclick="saveMetaToken()">Reintentar guardar</button><p class="notice">Se guarda automáticamente al pegarla. Nosotros no recibimos esta clave; queda local en esta instalación.</p></div></div><aside class="guide-checklist"><b>Qué hará la guía</b><ol><li>Crear o abrir tu app de Meta Business.</li><li>Crear un Usuario del sistema.</li><li>Dar acceso a tu cuenta publicitaria y página.</li><li>Generar una clave con permisos de anuncios y páginas.</li><li>Pegarla aquí y buscar tus cuentas.</li></ol></aside></section><div id="social-account-results" class="setup-guide"></div><div class="guide-panel"><b>Por qué esto es más seguro</b><p>La conexión queda entre tu Meta Business y tu instalación local. Si algún día quieres cortar acceso, quitas esa clave desde Meta y listo.</p></div></div>`;
 return `<div class="setup-guide private-connection"><section class="guide-hero"><div class="guide-main"><span class="guide-eyebrow">Secure step</span><h3>Connect my Facebook account</h3><p>The recommended path is a stable connection in your own Meta Business using a <strong>System User</strong>. This keeps access under your control and avoids relying on a temporary test token.</p><div class="guide-visual"><div class="mini-screen"><span></span><span></span><strong>1. Meta Business</strong><em>system user</em></div><div class="guide-arrow">&rarr;</div><div class="mini-screen"><span></span><span></span><strong>2. Stable token</strong><em>account and Page</em></div><div class="guide-arrow">&rarr;</div><div class="mini-screen"><span></span><span></span><strong>3. Local Admiro</strong><em>paste it here</em></div></div><div class="guide-support-grid"><div class="guide-card"><b>Recommended: stable connection</b><p>For daily use. Create a System User, assign your ad account and Page, generate the key, and paste it here.</p><div class="onboarding-step-actions"><a class="btn primary" href="/api/social/login" target="_blank" rel="noopener" onclick="connectMetaStarted()">Open Meta Business</a><button class="btn" type="button" onclick="showMetaTokenBox()">I have the stable key</button></div></div><div class="guide-card"><b>Quick test: temporary token</b><p>Only for testing. Graph API Explorer can expire and require reconnection. It is not ideal for leaving the agent working.</p><div class="onboarding-step-actions"><a class="btn" href="https://developers.facebook.com/tools/explorer/" target="_blank" rel="noopener">Open Graph API Explorer</a></div></div></div><div class="guide-actions"><button class="btn primary" type="button" onclick="refreshSocialAccounts()">Find my accounts</button><button class="btn" type="button" onclick="openChat('Help me connect Meta with a stable System User key. Explain it step by step in simple words.')">${t('ask_agent')}</button></div><div id="meta-token-box" class="token-box"><label>Stable Facebook/Meta key<textarea id="meta-token-input" oninput="scheduleMetaTokenAutoSave()" onpaste="setTimeout(scheduleMetaTokenAutoSave,0)" placeholder="Paste the stable key you generated in Meta Business"></textarea></label><button class="btn" type="button" onclick="saveMetaToken()">Retry save</button><p class="notice">It saves automatically when pasted. We do not receive this key; it stays local to this install.</p></div></div><aside class="guide-checklist"><b>What the guide does</b><ol><li>Create or open your Meta Business app.</li><li>Create a System User.</li><li>Give access to your ad account and Page.</li><li>Generate a key with ads and Page permissions.</li><li>Paste it here and find your accounts.</li></ol></aside></section><div id="social-account-results" class="setup-guide"></div><div class="guide-panel"><b>Why this is safer</b><p>The connection stays between your Meta Business and your local install. If you ever want to cut access, revoke that key from Meta.</p></div></div>`;
}
...
function connectMetaStarted(){showMetaTokenBox();toast(lang==='es'?'Meta Business se abrirá en otra pestaña. Sigue la guía y pega aquí tu clave estable.':'Meta Business will open in another tab. Follow the guide and paste your stable key here.')}
