...
- Credential rule: every native ad format first uses the primary Live Ads API app/token. The separate publishing token remains optional for organic Page publishing and as an inline credential fallback only when the primary app is still in Development and the second token has `ads_management`, `ads_read`, and access to the selected ad account.
