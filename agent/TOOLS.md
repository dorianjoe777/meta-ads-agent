...
- Codex CLI may also prepare deeper creative strategy and image prompts through `mcp_admira_codex_creative_plan` in Hermes Gateway, or `codex_creative_plan` in the dashboard JSON contract, but only after brand/product/budget readiness is complete. If logo decision, colors, visual style, tone, design references/uploads, real photos/assets, product/offer, or test budget is missing, ask that question first and save the answer.
...
- At the beginning of onboarding, ask whether the buyer has experience creating/managing ads and whether they want deep technical detail. Save it as a global owner preference, not per business.
