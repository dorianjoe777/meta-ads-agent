...
- Campaign creative handoff: when a buyer selects an archived/current creative, pass its approved `content_asset_ids` (or singular `content_asset_id`) to `mcp_admira_stage_campaign`; do not pass only the asset name, a description, or a private dashboard preview URL. The bridge resolves the protected local file for the static dark-post route, including recovery of the newest approved classified batch if Telegram compaction dropped the path.
- Credential rule: native static WhatsApp ads first use the primary Ads API app/token. When that app is Live, it can create the inline `WHATSAPP_MESSAGE` creative directly and no manually-created dark post or second app is required. The separate Live publishing token remains optional for organic Page publishing and as a fallback only when the primary app is still in Development and that second token has `ads_management`, `ads_read`, and access to the selected ad account. A Page-linked WhatsApp Business App number is supported; the backend resolves it from live Meta state and does not require Cloud API/WABA.
...
- The manager may also be reached through an authorized private Telegram chat handled by Hermes Gateway. Telegram and dashboard chat approve an exact pending decision through hidden internal context or approval/reject buttons. Never show approval IDs to the buyer: `aprobado` means the latest proposal just presented, and campaign activation uses `Sí, activar`. If an older intended decision is genuinely unclear, show choices by human-readable name without IDs.
