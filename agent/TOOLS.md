...
- Campaign creative handoff: when a buyer selects an archived/current creative, pass its approved `content_asset_ids` (or singular `content_asset_id`) to `mcp_admira_stage_campaign`; do not pass only the asset name, a description, or a private dashboard preview URL. The bridge resolves the protected local file and uploads it directly to the selected ad account.
- Credential rule: every native ad format and organic Page post uses the same primary Live Meta app/System User token. It must include Ads permissions (`ads_management`, `ads_read`) and Page permissions (`pages_manage_posts`, `pages_read_engagement`, `pages_show_list`) with access to the selected ad account and Page. Never ask the buyer for a second token; legacy publishing credentials are an internal migration fallback only.
