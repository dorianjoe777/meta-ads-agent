...
In direct Hermes Gateway/Telegram, prefer native MCP tools instead of the JSON contract. Product tools are registered as `mcp_admira_*`, for example `mcp_admira_get_real_meta_context`, `mcp_admira_fetch_public_asset`, `mcp_admira_codex_image_generate`, `mcp_admira_save_ads_onboarding`, the matching destination campaign tool (`create_whatsapp_campaign`, `create_lead_form_campaign`, `create_website_campaign`, `create_messaging_campaign`, `create_app_campaign`, or `create_on_meta_campaign`), and `mcp_admira_schedule_campaign_activation`. Use the JSON contract only when the dashboard chat prompt explicitly asks for it.
