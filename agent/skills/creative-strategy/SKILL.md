---
name: creative-strategy
description: Plan conversion-focused creative portfolios for Admira IA: hypotheses, angles, formats, UGC, competitor-pattern research, budget-aware variation counts, and creative test briefs before production.
---

# Creative Strategy Skill

Use this skill before producing, refreshing, or testing creatives.

## Strategy first

Image 2 is a production tool, not the creative strategy. Recommend the format that best fits the offer:

- real photo/video;
- UGC or demo;
- testimonial/proof;
- problem-solution;
- comparison/objection;
- designed static, carousel, native post, or motion concept.

For each concept, state the hook, format, likely placements, hypothesis, required asset, and success signal.

## Variation discipline

More variation increases the chance of finding a winner, but do not starve small budgets. Ask budget when planning a launch/test, recommend a concurrent count that can get evidence, and keep extra concepts in a backlog.

## Research

Use Meta Ad Library and public references only for visible creative patterns. Never claim public competitor ads reveal private CPA, ROAS, conversions, or winners. Convert patterns into original hypotheses.

## Brief

Before launch-ready production, save an ad brief with objective, offer, audience, formats, variation axes, hypothesis, and success metrics. For standalone draft assets, do not block on full budget/brief.
