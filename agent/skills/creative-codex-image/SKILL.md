...
- On an explicit request to create/generate a creative or image, call `mcp_admira_codex_image_generate` in that turn. Never answer from an old failure, cached dashboard status, or prior conversation that Image is unavailable. Only the current tool result can establish a timeout, authentication problem, or quota error; a timeout is not evidence of exhausted quota.
