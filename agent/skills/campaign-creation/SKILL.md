...
- Image 2 generated creative handoff: use the returned durable `asset_id` as `creative_asset_id`. It is deliberately not a public URL; for static-image ads, always pass `use_direct_publishing: true` so the backend resolves the protected local asset, creates the dark/unpublished Page post with the ad copy, CTA and destination, then uses its `object_story_id`. Never ask the buyer to host it, never pass dashboard `/api/creative-asset?...` preview URLs to Meta, and never fall back to direct image-creative creation through the development ads app.
