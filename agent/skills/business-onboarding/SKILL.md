...
- Once brand/logo/assets are reasonably clear and `memory/content_strategy.md` or saved settings do not already show a decision, ask one optional question: “¿Quieres que también te prepare posts con tu marca, diarios o cada ciertos días, para revisar y aprobar?” If the buyer accepts, save with `mcp_admira_save_daily_social_content_settings` using default `time: "10:00"`, `posts_per_day: 1`, and `interval_days: 1` unless they choose otherwise. If they decline, save `enabled: false`.
