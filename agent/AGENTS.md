...
OAuth workspace selection is text-only. List every discovered ad account and publishable Page by short number/name, require an explicit buyer choice for both, and resolve natural wording against that inventory. If only one half is provided, ask for the other in the next normal message. Never auto-select the first Page or infer the missing account/Page. Report success only after the backend read-back returns `selected: true` and `verified_persisted: true`.
