...
async function load(){state=await api('/api/dashboard');render();if(!uiWorkbenchPreview&&state.config.dashboard_password_required&&!state.config.dashboard_password_set)hideUnlock();else if(!uiWorkbenchPreview&&state.config.dashboard_password_required&&state.config.dashboard_password_set&&!dashboardPassword()&&state.onboarding&&state.onboarding.completed)showUnlock(t('unlock_needed'),'unlock');checkForUpdates(false);startUpdateAutoCheck();setTimeout(startDashboardIntroTourIfPending,350)}
...
function copyVisibleChatGptCode(event){
 const source=event?.target||event?.currentTarget||null;
 const visible=source?.closest?.('.chatgpt-device-code')?.querySelector('strong')||qs('#chatgpt-connect-result .chatgpt-device-code strong');
...
function telegramStatusMarkup(value={}){
 ...
 if(v.bot_configured)return `<div class="telegram-next-action"><div class="telegram-orb">AI</div><div><b>${lang==='es'?'Clave guardada':'Key saved'}</b><p>${lang==='es'?'Ahora envía un "hola" al bot que creaste. Después vuelve aquí y detecto tu chat privado.':'Now send "hello" to the bot you created. Then come back here and I will detect your private chat.'}</p></div><button class="btn primary telegram-detect-button" type="button" data-action-code="detectTelegramChats()">${lang==='es'?'Ya envié hola, detectar mi chat':'I sent hello, detect my chat'}</button></div>`;
...
async function autoSaveTelegramToken(event){
 ...
 telegramAutoSaveTimer=setTimeout(async()=>{
  try{
   ...
   toast(lang==='es'?'Clave de Telegram guardada':'Telegram key saved');
  }catch(err){
...
async function detectTelegramChats(){
 ...
 box.innerHTML=`<div class="telegram-chat-choices"><div class="telegram-chat-choice-head"><b>${lang==='es'?'Encontré estos chats':'I found these chats'}</b><p>${lang==='es'?'Elige el tuyo. Solo ese chat quedará autorizado para hablar con el agente.':'Choose yours. Only that chat will be authorized to talk with the agent.'}</p></div>${rows.map(c=>`<div class="telegram-chat-choice"><div><span>${lang==='es'?'Chat detectado':'Detected chat'}</span><b>${escapeHtml(c.label)} ${escapeHtml(c.username||'')}</b><p>ID ${escapeHtml(c.id)}</p></div><button class="btn primary" type="button" data-action-code="selectTelegramChat('${escapeHtml(c.id)}',qs('#onboarding-flow')?.classList.contains('open'))">${lang==='es'?'Usar este chat y terminar':'Use this chat and finish'}</button></div>`).join('')}</div>`;
}
...
function accountChoiceCard(account){
 ...
 const meta=[account.id,account.currency].filter(Boolean).join(' · ');
 return `<article class="found-choice-card ad-account-choice"><div><span class="choice-kicker">${lang==='es'?'Cuenta encontrada':'Account found'}</span><b>${escapeHtml(name)}</b><p>${escapeHtml(meta)}</p></div><button class="btn primary" type="button" data-action-code="selectSocialAccount(${chatArg(account.id)})">${lang==='es'?'Usar esta cuenta y seguir':'Use this account and continue'}</button></article>`;
}
function renderSocialAccountResults(res){
 ...
 if(res.accounts&&res.accounts.length){
  ...
  box.innerHTML=`<div class="guide-panel found-summary"><b>${single?(lang==='es'?'Encontré tu cuenta publicitaria':'I found your ad account'):(lang==='es'?'Elige tu cuenta publicitaria':'Choose your ad account')}</b><p>${single?(lang==='es'?'Haz clic en el botón rosa y seguimos al siguiente paso.':'Click the pink button and we continue to the next step.'):(lang==='es'?'Elige la cuenta real que quieres cuidar con Admira IA.':'Choose the real account you want Admira IA to manage.')}</p></div><div class="account-choice-grid">${res.accounts.map(accountChoiceCard).join('')}</div>`;
