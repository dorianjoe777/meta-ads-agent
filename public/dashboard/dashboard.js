...
function allowedActionCall(name){
 const actions={
  ...
  copyCommand,copyVisibleChatGptCode,skipWebsiteScan,setBusinessContextQuestionIndex,setMetaGuideSlide,setMetaGuideFrame,skipOnboarding,closeUsageGuide,finishDashboardIntroTour,
...
function renderMetaTokenSlide(){
 ...
 const imageMarkup=frame?`<img src="${metaAssetSrc(frame.src)}" alt="${escapeHtml(frame.label||s.title)}" loading="lazy">${frames.length>1?`<div class="meta-frame-caption"><span>${metaGuideFrame+1}/${frames.length}</span><b>${escapeHtml(frame.label||s.shot)}</b></div>`:''}`:`<div><span>${escapeHtml(s.shot)}</span><p>${lang==='es'?'Aquí irá la captura real de este paso.':'The real screenshot for this step will go here.'}</p></div>`;
