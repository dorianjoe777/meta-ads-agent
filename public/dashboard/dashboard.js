...
function dataSourceText(m){const source=String(m?.source||'');if(source==='meta_graph')return lang==='es'?'Meta en vivo hoy · se actualiza cada 2 min':'Live Meta today · refreshes every 2 min';if(source==='demo')return lang==='es'?'Datos de ejemplo, no reales':'Demo data, not real';if(source==='missing')return lang==='es'?'Sin datos reales de Meta':'No real Meta data yet';return lang==='es'?'Datos guardados sin confirmar':'Saved unverified data'}
