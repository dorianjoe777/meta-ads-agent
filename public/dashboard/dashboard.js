...
async function openUpdateFromUrl(){if(urlParams.get('open_update')!=='1')return;activateDashboardTab('setup');urlParams.delete('open_update');const next=urlParams.toString();history.replaceState({},'',window.location.pathname+(next?'?'+next:'')+window.location.hash);setTimeout(()=>qs('#update-rollback-panel')?.scrollIntoView({behavior:'smooth',block:'center'}),180);await checkForUpdates(true,{silent:true});if(updateInfo?.available)showUpdateDetails()}
