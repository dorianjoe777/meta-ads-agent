...
async function refreshInsights(options={}){if(liveMetricsRefreshInFlight)return null;liveMetricsRefreshInFlight=true;try{const res=await api('/api/action',{method:'POST',body:JSON.stringify({action:'refresh_insights'})});if(!options.silent){if(res.result&&res.result.ok){toast(lang==='es'?'Datos reales actualizados desde Meta.':'Real Meta data refreshed.')}else{toast(lang==='es'?'No pude leer datos reales todavía. Revisa tu clave de Meta y la cuenta elegida.':'Could not read real data yet. Check your Meta key and chosen account.')}}if(res.result&&res.result.ok)await load();return res}finally{liveMetricsRefreshInFlight=false}}
function startLiveMetricsAutoRefresh(){if(uiWorkbenchPreview||liveMetricsAutoTimer)return;liveMetricsAutoTimer=setInterval(()=>refreshInsights({silent:true}),10*60*1000)}
