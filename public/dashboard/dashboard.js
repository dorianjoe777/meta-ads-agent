...
function stepCopy(key){
	 const en={
	  ...
	  destination:['Pick where ads go','Choose the Facebook Page, Instagram, and website. The single Meta token also enables organic Page publishing.',''],
...
function onboardingSteps(){
 ...
 const model=state.config.agent_model||{};
 const studio=state.config.creative_studio||{};
 const brain=model.brain_provider||'gemini';
 const apiBrainOk=['gemini','openai_api','minimax','custom_api'].includes(brain)&&model.api_key_set&&Boolean(model.base_url)&&Boolean(model.model);
...
function metaTokenSlides(){
 ...
 const es=[
  ...
  {title:'Marca permisos de la clave',shot:'Paso 9',images:[metaFrame('tutorial-meta/meta-business-26-token-permissions-top.png','Marca anuncios y negocio'),metaFrame('tutorial-meta/meta-business-27-token-permissions-middle.png','Marca permisos de página'),metaFrame('tutorial-meta/meta-business-28-token-permissions-page.png','Permite publicar desde Telegram')],body:'Marca los permisos de anuncios y de Página porque esta única clave se usará para ambos.',items:['Necesarios: ads_management, ads_read, business_management y pages_manage_ads.','También marca pages_show_list y pages_read_engagement.','Marca pages_manage_posts para publicar contenido orgánico desde el agente.']},
 ...
 const en=[
  ...
  {title:'Select token permissions',shot:'Step 9',images:es[8].images,body:'Select both Ads and Page permissions because this single key handles both jobs.',items:['Required: ads_management, ads_read, business_management, and pages_manage_ads.','Also select pages_show_list and pages_read_engagement.','Select pages_manage_posts to publish organic content from the agent.']},
...
function metaConnectionGuide(){
 ...
 const status=connected
  ? (lang==='es'?'Facebook está conectado. Hermes ya recibió las cuentas y la Página seleccionadas.':'Facebook is connected. Hermes already received the selected accounts and Page.')
  : (lang==='es'?'Facebook se conecta únicamente desde Hermes por Telegram. El agente te enviará el enlace y te pedirá elegir la cuenta y la Página allí.':'Facebook is connected only through Hermes in Telegram. The agent will send the link and ask you to choose the account and Page there.');
 return `<div class="setup-guide private-connection"><section class="guide-hero"><div class="guide-main"><span class="guide-eyebrow">${lang==='es'?'Conexión gestionada por Hermes':'Hermes-managed connection'}</span><h3>${lang==='es'?'Facebook se configura desde Telegram':'Facebook is configured from Telegram'}</h3><p>${status}</p><div class="guide-panel"><b>${connected?(lang==='es'?'Estado: conectado':'Status: connected'):(lang==='es'?'Siguiente paso: abre Telegram':'Next step: open Telegram')}</b><p>${lang==='es'?'No hay botones ni campos de Facebook en este dashboard. Usa la conversación con Hermes para iniciar, cambiar o seleccionar cuentas.':'This dashboard has no Facebook buttons or fields. Use the Hermes conversation to connect, change, or select accounts.'}</p></div></div><aside class="guide-checklist"><b>${lang==='es'?'Qué hará Hermes':'What Hermes does'}</b><ol><li>${lang==='es'?'Te envía el enlace seguro.':'Sends the secure link.'}</li><li>${lang==='es'?'Te guía para aceptar permisos.':'Guides you through permissions.'}</li><li>${lang==='es'?'Te pide elegir cuenta y Página en Telegram.':'Asks you to choose the account and Page in Telegram.'}</li></ol></aside></section><p class="notice">${lang==='es'?'La conexión se guarda de forma privada en tu instalación y se sincroniza con Hermes.':'The connection is stored privately in your installation and synchronized with Hermes.'}</p></div>`;
}
...
function directPublishingState(){
 ...
 const oauth=state.config?.meta_oauth||{};
 const oauthConnected=Boolean(oauth.connected);
 const oauthFirst=Boolean(oauth.broker_configured||oauthConnected);
 ...
 return {v,publishing,oauth,oauthConnected,oauthFirst,tokenSet,ready,page,savedAt,title,body,badge,placeholder};
}
function directPublishingGuide(onboarding=false){
 const s=directPublishingState();
 if(s.oauthFirst){
  return `<div class="setup-guide direct-publishing-guide"><section class="guide-hero"><div class="guide-main"><span class="guide-eyebrow">${lang==='es'?'Publicación directa':'Direct publishing'}</span><h3>${lang==='es'?'Hermes gestiona Facebook y la Página':'Hermes manages Facebook and the Page'}</h3><p>${s.oauthConnected?(lang==='es'?'La conexión ya está lista para anuncios y publicaciones orgánicas.':'The connection is ready for ads and organic posts.'):(lang==='es'?'La conexión se inicia y se cambia únicamente desde Telegram.':'The connection is started and changed only through Telegram.')}</p><div class="guide-panel"><b>${lang==='es'?'Sin configuración manual aquí':'No manual setup here'}</b><p>${lang==='es'?'Habla con Hermes para conectar Facebook, cambiar de negocio o revisar la Página activa.':'Talk to Hermes to connect Facebook, change business, or review the active Page.'}</p></div></div><aside class="guide-checklist"><b>${lang==='es'?'Qué habilita':'What it enables'}</b><ol><li>${lang==='es'?'Crear y leer anuncios.':'Create and read ads.'}</li><li>${lang==='es'?'Publicar posts orgánicos.':'Publish organic posts.'}</li><li>${lang==='es'?'Elegir la cuenta desde Telegram.':'Choose the account from Telegram.'}</li></ol></aside></section></div>`;
 }
 const appsUrl='https://business.facebook.com/latest/settings/apps';
 const usersUrl='https://business.facebook.com/latest/settings/system-users';
 ...
 const markup=`<div class="setup-guide direct-publishing-guide"><section class="guide-hero"><div class="guide-main"><span class="guide-eyebrow">${lang==='es'?'Publicación directa':'Direct publishing'}</span><h3>${lang==='es'?'Una sola conexión para anuncios y Página':'One connection for ads and your Page'}</h3><p>${escapeHtml(permissionNote)}</p><div class="guide-actions"><a class="btn" href="${appsUrl}" target="_blank" rel="noopener noreferrer">${lang==='es'?'Abrir Apps':'Open Apps'}</a><a class="btn" href="${usersUrl}" target="_blank" rel="noopener noreferrer">${lang==='es'?'Abrir System Users':'Open System Users'}</a><button class="btn" type="button" data-action-code="testPublishingConnection()">${lang==='es'?'Revisar conexión':'Check connection'}</button></div></div><aside class="guide-checklist"><b>${lang==='es'?'Qué habilita':'What it enables'}</b><ol><li>${lang==='es'?'Crear y leer anuncios.':'Create and read ads.'}</li><li>${lang==='es'?'Publicar posts orgánicos en tu Página.':'Publish organic posts to your Page.'}</li><li>${lang==='es'?'Usar el mismo token sin una segunda app o clave.':'Use the same token without a second app or key.'}</li></ol></aside></section><div class="guide-panel"><b>${lang==='es'?'Token unificado':'Unified token'}</b><p>${escapeHtml(lang==='es'?'No pegues un segundo token aquí: el campo principal de Meta es el único que se usa. Si cambias la cuenta, reemplázalo allí y vuelve a descubrir tus páginas.':'Do not paste a second token here: the main Meta field is the only one used. If you change accounts, replace it there and rediscover your Pages.')}</p><span class="status-badge">${escapeHtml(s.badge)}</span></div></div>`;
 return markup;
}
...
function compactAgentSetup(){
 const model=state.config.agent_model||{};
 const studio=state.config.creative_studio||{};
 // Google AI Studio is the simple first-run brain; subscription routes remain available.
 const brain=model.brain_provider||'gemini';
 const apiProviders=['gemini','openai_api','minimax','custom_api'];
 const provider=brain==='openai_codex'?'openai_codex':(apiProviders.includes(brain)?brain:'gemini');
 ...
 const defaults={
  ...
  gemini:{base:'https://generativelanguage.googleapis.com/v1beta',model:'gemini-3.5-flash-lite'},
 ...
 return `<form id="agent-model-form" class="activation-form" data-submit-code="saveSetupConfig(event)">
  ...
  <div class="activation-model-grid">
   ...
   <option value="gemini" ${provider==='gemini'?'selected':''}>Google AI Studio · Gemini</option>
   ...
   <label id="compact-agent-model-field" class="${isChatGpt&&!chatgptConnected?'hidden':''}">${lang==='es'?'Modelo':'Model'}<input name="agent_chat_model" value="${escapeHtml(modelName)}" placeholder="${isChatGpt?(lang==='es'?'Conecta ChatGPT primero':'Connect ChatGPT first'):(lang==='es'?'Nombre del modelo':'Model name')}" ${isChatGpt&&!chatgptConnected?'disabled':''}></label>
...
function selectCompactAgentProvider(event){
 ...
 const provider=String(picked||event.target.value||'gemini');
 const defaults={
...
function chatGptConnectMarkup(onboarding=false){
 ...
 const model=state.config.agent_model||{};
 const studio=state.config.creative_studio||{};
 // Google AI Studio is the default API route for new installations.
 const brain=model.brain_provider||'gemini';
 const apiBrain=['gemini','openai_api','minimax','custom_api'].includes(brain);
 ...
 const draft=lang==='es'?'Ayúdame a elegir el cerebro del agente. Explícame en palabras simples si me conviene Google AI Studio, ChatGPT/Codex, MiniMax M3 u otra API.':'Help me choose the agent brain. Explain simply whether Google AI Studio, ChatGPT/Codex, MiniMax M3, or another API is better for me.';
 const savedBase=model.base_url||'';
 const primaryRoute=brain==='openai_codex'?'chatgpt_subscription':(brain==='gemini'||savedBase.includes('generativelanguage.googleapis.com')?'google_ai_studio':(brain==='minimax'||savedBase.includes('minimax')?'minimax_m3':(brain==='openai_api'||savedBase.includes('api.openai.com')?'openai_api':'custom_api')));
 const selectedRoute=primaryRoute;
 const connections=model.connections||{};
 const providerForRoute={google_ai_studio:'gemini',openai_api:'openai_api',minimax_m3:'minimax',custom_api:'custom_api'};
 ...
 const base=selectedConnection.base_url||model.base_url||(selectedRoute==='google_ai_studio'?'https://generativelanguage.googleapis.com/v1beta':(selectedRoute==='openai_api'?'https://api.openai.com/v1':(selectedRoute==='custom_api'?'':'https://api.minimax.io/v1')));
 const modelName=selectedConnection.model||model.model||(selectedRoute==='google_ai_studio'?'gemini-3.5-flash-lite':(selectedRoute==='openai_api'?'gpt-4.1-mini':(selectedRoute==='custom_api'?'':'MiniMax-M3')));
 ...
 const routeCopy={
  ...
  google_ai_studio:{icon:'GA',title:'Google AI Studio · Gemini',desc:lang==='es'?'Recomendado · pega tu API key de Google AI Studio.':'Recommended · paste your Google AI Studio API key.',panel:lang==='es'?'Pega tu API key de Google AI Studio. El modelo inicial es Gemini 3.5 Flash Lite.':'Paste your Google AI Studio API key. The default model is Gemini 3.5 Flash Lite.'},
 ...
 const routeConnected={chatgpt_subscription:chatgptConnected,google_ai_studio:Boolean(connections.gemini?.configured),openai_api:Boolean(connections.openai_api?.configured),minimax_m3:Boolean(connections.minimax?.configured),custom_api:Boolean(connections.custom_api?.configured)};
 ...
 const apiModelField=`<input name="agent_chat_model" value="${escapeHtml(modelName)}" placeholder="${lang==='es'?'Nombre del modelo':'Model name'}">`;
 const nvidiaCatalogAction=selectedRoute==='google_ai_studio'?`<a class="btn" href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer">${lang==='es'?'Obtener API key de Google AI Studio':'Get a Google AI Studio API key'}</a>`:'';
 ...
 <div class="agent-route-panels">
  ...
  <div class="agent-route-panel ${selectedRoute!=='chatgpt_subscription'?'active':''}" data-agent-route-panel="api"><h4 id="agent-api-route-title">${apiPanelTitle}</h4><p id="agent-api-route-help">${apiPanelHelp}</p><div class="form-grid">
   ...
   <div class="field"><label>${lang==='es'?'URL compatible OpenAI':'OpenAI-compatible URL'}</label><span class="field-help">${selectedRoute==='google_ai_studio'?(lang==='es'?'Endpoint oficial fijo de Google AI Studio.':'Fixed official Google AI Studio endpoint.'):(lang==='es'?'Debe usar https://. Solo se permite http:// para modelos locales como 127.0.0.1.':'Must use https://. http:// is allowed only for local models such as 127.0.0.1.')}</span><input name="agent_chat_base_url" value="${escapeHtml(base)}" placeholder="https://api.ejemplo.com/v1" ${selectedRoute==='google_ai_studio'?'readonly':''}></div>
...
function applyAgentModelPreset(kind){
 ...
 const defaults={
  ...
  google_ai_studio:{base_url:'https://generativelanguage.googleapis.com/v1beta',model:'gemini-3.5-flash-lite'},
...
function selectAgentModelRoute(kind){
 ...
 if(modelInput)modelInput.removeAttribute('list');
 if(baseInput)baseInput.readOnly=route==='google_ai_studio';
 ...
 const copy={
  ...
  google_ai_studio:{title:'Google AI Studio · Gemini',help:lang==='es'?'Pega tu API key de Google AI Studio. Gemini 3.5 Flash Lite será el modelo predeterminado.':'Paste your Google AI Studio API key. Gemini 3.5 Flash Lite is the default model.'},
