...
function renderPublishingPanel(){
 ...
 const body=ready?(lang==='es'?'El agente puede crear posts ocultos/nativos para anuncios y dejar publicaciones listas para aprobar.':'The agent can create native unpublished posts for ads and prepare social posts for approval.'):(tokenSet?(lang==='es'?'Clave guardada. Revisa que tenga acceso a la página antes de publicar desde el agente.':'Key saved. Check that it can access the Page before publishing from the agent.'):(lang==='es'?'Opcional: conecta la app Live de publicaciones para que Admira pueda crear posts, creativos y publicaciones aprobables desde Telegram.':'Optional: connect the Live publishing app so Admira can create posts, creatives, and approval-ready social content from Telegram.'));
 ...
 box.innerHTML=`<div class="next-step meta-connection-card"><div><b>${title}</b><p>${body}</p>${savedAt?`<p class="notice">${lang==='es'?'Guardada':'Saved'}: ${escapeHtml(savedAt)}</p>`:''}</div><span class="badge ${ready?'ok':(tokenSet?'warn':'')}">${badge}</span></div>
 <div class="trust-grid license-limits-grid"><div class="trust-card"><b>${lang==='es'?'Página':'Page'}</b><p>${escapeHtml(page||'-')}</p></div><div class="trust-card"><b>${lang==='es'?'Uso':'Use'}</b><p>${lang==='es'?'Posts diarios, anuncios con post nativo y aprobación simple.':'Daily posts, native-post ads, and simple approval.'}</p></div><div class="trust-card"><b>${lang==='es'?'Privacidad':'Privacy'}</b><p><a href="https://admiroia.uboost.lat/privacy" target="_blank" rel="noopener noreferrer">admiroia.uboost.lat/privacy</a></p></div></div>
 <form id="publishing-config-form" class="form-grid">
  <div class="field wide"><label>${lang==='es'?'Clave de la app Live de publicaciones':'Live publishing app key'}</label><span class="field-help">${lang==='es'?'Debe tener pages_manage_posts y pages_read_engagement. Se guarda solo en este equipo/VPS.':'Needs pages_manage_posts and pages_read_engagement. Stored only on this PC/VPS.'}</span><input type="password" name="token" value="" placeholder="${escapeHtml(placeholder)}" autocomplete="off"></div>
  <div class="field wide"><div class="mode-actions"><button class="btn primary" type="submit">${tokenSet?(lang==='es'?'Cambiar clave':'Replace key'):(lang==='es'?'Guardar publicación directa':'Save direct publishing')}</button><button class="btn" type="button" data-action-code="testPublishingConnection()">${lang==='es'?'Revisar conexión':'Check connection'}</button>${tokenSet?`<button class="btn danger" type="button" data-action-code="disconnectPublishingConfig()">${lang==='es'?'Desconectar':'Disconnect'}</button>`:''}</div></div>
 </form>`;
 qs('#publishing-config-form')?.addEventListener('submit',savePublishingConfig);
}
...
function card(c){const accountLabel=c.account_name||c.ad_account_id||c.account_id||'';const accountNote=accountLabel?`<p class="notice">${lang==='es'?'Cuenta':'Account'}: ${escapeHtml(accountLabel)}</p>`:'';const draft=lang==='es'?`Analiza la campaña ${c.name}. Está como ${statusText(c.health)} con ROAS ${Number(c.roas).toFixed(2)}x y CPA ${fmtMoney(c.cpa)}. ¿Qué harías como manager?`:`Analyze campaign ${c.name}. It is ${statusText(c.health)} with ROAS ${Number(c.roas).toFixed(2)}x and CPA ${fmtMoney(c.cpa)}. What would you do as manager?`;return `<article class="card aurora-card" data-health="${c.health}"><span class="starfield" aria-hidden="true"></span><div class="top"><h3>${escapeHtml(demoCampaignName(c.name))}</h3><span class="badge ${c.health}">${statusText(c.health)}</span></div>${accountNote}<div class="metrics">${metric('Spend',fmtMoney(c.spend))}${metric('ROAS',Number(c.roas).toFixed(2)+'x')}${metric('CPA',fmtMoney(c.cpa))}${metric('CTR',fmtPct(c.ctr))}</div>${spark(c.trend)}<div class="actions">${campaignButtons(c)}<button class="btn ask-btn" data-action-code="openChat(${JSON.stringify(draft).replaceAll('"','&quot;')})">${t('ask_agent')}</button></div></article>`}
...
function accountLimitMessage(account){
 ...
 if(reason==='business_manager_mismatch')return lang==='es'?'Pertenece a otro Business Manager':'Different Business Manager';
 if(reason==='business_manager_unknown')return lang==='es'?'Meta no confirmó el Business Manager':'Business Manager not confirmed';
...
function accountChoiceCard(account){
 ...
 const action=canSelect?`data-action-code="selectSocialAccount(${chatArg(encodeAccountChoice(account))})"`:'disabled';
 const label=account.managed?(lang==='es'?'Usar esta cuenta y seguir':'Use this account and continue'):(lang==='es'?'Agregar esta cuenta y seguir':'Add this account and continue');
 return `<article class="found-choice-card ad-account-choice"><div><span class="choice-kicker">${escapeHtml(status)}</span><b>${escapeHtml(name)}</b><p>${escapeHtml(meta)}</p></div><button class="btn primary" type="button" ${action}>${label}</button></article>`;
}
...
async function selectSocialAccount(choice){const account=decodeAccountChoice(choice);const id=account.id||account.ad_account_id||'';const payload={ad_account_id:id,account_name:account.name||'',account_currency:account.currency||'',account_status:account.status||'',business_manager_id:account.business_id||account.business_manager_id||'',business_manager_name:account.business_name||account.business_manager_name||''};try{await api('/api/social/default-account',{method:'POST',body:JSON.stringify(payload)})}catch(err){if(needsBusinessReplacement(err)){showBusinessReplacementConfirm(payload);return}throw err}const input=qs('input[name="ad_account_id"]');if(input)input.value=id;toast(lang==='es'?'Cuenta guardada. Buscando perfiles conectados...':'Account saved. Finding connected assets...');const discovered=await discoverMetaAssets(id);try{await api('/api/action',{method:'POST',body:JSON.stringify({action:'refresh_insights'})})}catch(err){}await load();const steps=onboardingSteps();const destinationIndex=steps.findIndex(s=>s.id==='destination');if(destinationIndex>=0){onboardingFlowTouched=true;onboardingFlowStep=destinationIndex;renderOnboardingFlow();renderDiscoveredAssets(discovered)}else advanceOnboardingAfterLoad()}
