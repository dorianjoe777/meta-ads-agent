...
function copyVisibleChatGptCode(source){
 const trigger=source?.target?.closest?.('[data-action-code]')||source?.closest?.('[data-action-code]')||source||null;
 const card=trigger?.closest?.('.chatgpt-device-code')||qs('#chatgpt-connect-result .chatgpt-device-code');
 let code=normalizeChatGptCode(card?.querySelector?.('[data-chatgpt-visible-code]')?.textContent||'');
 if(!code)code=normalizeChatGptCode(card?.querySelector?.('strong')?.textContent||'');
 if(!code){
  const visibleCodes=Array.from(document.querySelectorAll('#chatgpt-connect-result .chatgpt-device-code [data-chatgpt-visible-code], #chatgpt-connect-result .chatgpt-device-code strong')).filter(el=>el.offsetParent!==null);
...
function renderChatGptConnectResult(response){
 ...
 const codeBlock=loginCode?`<div class="chatgpt-device-code" role="status" aria-live="polite"><div><span>${lang==='es'?'Código para OpenAI':'Code for OpenAI'}</span><strong data-chatgpt-visible-code>${escapeHtml(loginCode)}</strong><small>${lang==='es'?'Pégalo en la pestaña de OpenAI/Codex que se abrió. Si ChatGPT muestra un error en rojo, toca el botón de ayuda.':'Paste it in the OpenAI/Codex tab that opened. If ChatGPT shows a red error, click the help button.'}</small></div><div class="chatgpt-device-actions"><button class="btn primary" type="button" data-action-code="copyVisibleChatGptCode(source)">${lang==='es'?'Copiar código':'Copy code'}</button><button class="btn" type="button" data-action-code="toggleChatGptDeviceAuthHelp()">${lang==='es'?'Haz clic aquí si te apareció un error':'Click here if you saw an error'}</button></div></div>${chatGptDeviceAuthHelpMarkup()}`:'';
