...
The provider compatibility layer always replaces any cached Admira campaign
schema with the current one-field `brief_markdown` contract, including a stale
schema that is non-empty. One destination request should contain one complete
brief. A compiler validation miss may cause at most one corrected call, and
that retry must repeat the entire confirmed brief rather than sending only the
last missing fields.

When the buyer explicitly chooses an existing creative, the current turn keeps
recent-creative lookup available but removes image/video generation. This is
contextual: generation remains available during campaign conversations when
the buyer actually asks for a new or revised creative.
...
Natural geography remains exact across the compiler boundary. A value such as
`["Cartagena de Indias, Colombia"]` is split into the country constraint plus a
live city query; that query survives every normalization pass and must be
verified against the city key Graph persisted. Natural all-gender wording such
as "hombres y mujeres" is canonicalized to Meta `[1,2]`, while genuinely
unknown wording still blocks before mutation.
