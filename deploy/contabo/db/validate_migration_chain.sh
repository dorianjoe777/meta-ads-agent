...
expected=(007_trial_provider_lifecycle.sql 008_central_image_jobs.sql 009_telegram_license_recovery.sql 010_operator_gemini_pool.sql 011_operator_dashboard.sql 012_personal_chatgpt_sponsorship.sql 013_operator_trial_provisioning.sql 014_telegram_typing_indicator.sql 015_telegram_typing_retry_continuity.sql 016_central_campaign_compiler.sql)
...
# lifecycle validators there after applying 001-014.
if (( failures > 0 )); then
  exit 1
fi
printf '%s\n' 'Migration chain 007-016 passed read-only checks; no database was changed.'
