...
expected=(007_trial_provider_lifecycle.sql 008_central_image_jobs.sql 009_telegram_license_recovery.sql 010_operator_gemini_pool.sql)
...
# psql at a disposable clone, never at the live database, and run the four
# existing validators there after applying 001-010.
if (( failures > 0 )); then
  exit 1
fi
printf '%s\n' 'Migration chain 007-010 passed read-only checks; no database was changed.'
