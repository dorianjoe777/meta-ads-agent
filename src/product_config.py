...
def agent_model_connections(config=None, include_secrets=False):
    ...
    if legacy_key and not legacy_provider:
        ...
        elif "generativelanguage.googleapis.com" in legacy_url:
...
def persisted_buyer_timezone(default=""):
    """Return the durable buyer/account timezone when one was resolved.

    Dashboard settings live on a Docker volume while Compose environment
    values can remain installation defaults. Reading this small state file
    prevents a container recreation from reverting an explicit buyer choice
    or a verified selected-ad-account timezone back to UTC.
    """
    try:
        payload = json.loads(TIMEZONE_PREFERENCE_FILE.read_text(encoding="utf-8"))
    except (OSError, ValueError, TypeError):
        payload = {}
    timezone_name = normalize_timezone((payload or {}).get("timezone"), default="")
    return timezone_name or str(default or "").strip()
...
class AgentConfig:
    ...
    agent_brain_provider: str = "gemini"
...
def load_config():
    ...
    brain_provider = normalize_agent_brain_provider(
        env_first("AGENT_BRAIN_PROVIDER", default="gemini"),
    ...
    return AgentConfig(
        ...
        daily_brief_timezone=normalize_timezone(
            persisted_buyer_timezone(env_first("DAILY_BRIEF_TIMEZONE", "TZ", default="UTC"))
        ),
        ...
        agent_chat_api_key=env_first("AGENT_CHAT_API_KEY", "GEMINI_API_KEY", "MINIMAX_API_KEY", default=""),
        ...
        ) if brain_provider == "nvidia_nim" else (
            env_first("AGENT_CHAT_MODEL", "GEMINI_MODEL", default="gemini-3.5-flash-lite")
            if brain_provider == "gemini"
            else env_first("AGENT_CHAT_MODEL", "ADMIRA_NVIDIA_MODEL", default="")
        ),
