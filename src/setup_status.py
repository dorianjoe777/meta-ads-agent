...
def meta_section(config, destination):
    return [
        ...
        item("publishing_token", "Direct publishing key", "ok" if configured(getattr(config, "meta_publishing_access_token", "")) else "warn", "configured" if configured(getattr(config, "meta_publishing_access_token", "")) else "Opcional: conecta la app Live de publicaciones para crear posts y creativos nativos desde el agente.", "Conecta Publicación directa desde el onboarding o Configuración si quieres aprobar/publicar posts y evitar bloqueos de creative por app en desarrollo."),
