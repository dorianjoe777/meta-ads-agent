...
def meta_section(config, destination):
    return [
        ...
        item("publishing_token", "Direct publishing key", "ok" if configured(getattr(config, "meta_publishing_access_token", "")) else "warn", "configured" if configured(getattr(config, "meta_publishing_access_token", "")) else "Opcional: conecta la app Live para publicación orgánica y fallback de posts.", "Conecta Publicación directa desde onboarding o Configuración si quieres publicar en la Página. Los anuncios nativos de WhatsApp usan primero la app principal de Ads cuando está Live."),
