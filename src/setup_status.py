...
def agent_chat_section(config, agent_profile):
    ...
    entries = [
        item("chat_provider", "Agent chat provider", "ok" if config.agent_chat_provider == "hermes" else "warn", config.agent_chat_provider, "Recommended provider: hermes."),
        item("hermes_runtime", "Hermes runtime", "ok" if (hermes_cli or hermes_library) else "blocked", hermes_cli or ("Python library installed" if hermes_library else "Hermes not installed"), "Install Hermes, then run hermes model and choose OpenAI Codex / ChatGPT OAuth."),
        item("hermes_auth", "Hermes ChatGPT/Codex login", "ok" if hermes_auth["ready"] else "blocked", hermes_auth["detail"], "Run hermes model, choose OpenAI Codex, and sign in with the buyer's ChatGPT subscription."),
        item("chat_model", "Agent chat model", "ok", config.hermes_model or "Hermes selected model"),
        item("minimax_fallback", "MiniMax fallback", "ok" if configured(config.agent_chat_api_key) else "warn", "configured" if configured(config.agent_chat_api_key) else "Optional fallback not configured", "Only needed if you set AGENT_CHAT_PROVIDER=minimax."),
    ...
    return entries, {"hermes_cli": hermes_cli, "hermes_library": hermes_library, "hermes_auth": hermes_auth}
...
def setup_summary(config, sections, context):
    ...
    return {
        ...
        "agent_chat_ready": config.agent_chat_provider == "hermes" and bool(context["hermes_cli"] or context["hermes_library"]) and context["hermes_auth"]["ready"],
