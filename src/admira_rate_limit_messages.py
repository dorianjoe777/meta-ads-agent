...
def gateway_rate_limit_reply(text, language="es"):
    ...
    if provider in {"gemini", "google", "google-ai-studio", "google-ai-studio-api"}:
        if english:
            base = "⏱️ Google AI Studio/Gemini reached a temporary usage limit."
            reset = f" Try again in about {hint}." if hint else " Try again later; Google did not provide an exact reset time."
            return base + reset + " Your Facebook connection and saved work are safe."
        base = "⏱️ Google AI Studio/Gemini alcanzó un límite temporal de uso."
        reset = f" Intenta de nuevo en aprox. {hint}." if hint else " Intenta de nuevo más tarde; Google no indicó una hora exacta de reinicio."
        return base + reset + " Tu conexión de Facebook y tu trabajo guardado siguen seguros."
