...
def validate_meta_targeting_selection(interests=None, locations=None, age_min=18, age_max=65, live_search=None, verify_locations=True):
    ...
    for item in locations or []:
        ...
        if location_type == "country" or (len(key) == 2 and key.isalpha()):
            ...
            if not name or name.upper() == key:
                name = country_name_for_code(country_code)
        ...
        if live_search and verify_locations:
            ...
            def location_matches(row):
                ...
                if location_type == "country":
                    return row_key.upper() == key.upper() or bool(
                        country_code and row_country == country_code
                    )
                # A city/region key is the authoritative live selection. A
                # same-country match is not sufficient: it previously let a
                # fabricated Cartagena key pass merely because another search
                # result was located in Colombia.
                return row_key == key
...
def normalize_gender_values(value):
    ...
    for item in raw:
        ...
        paired_gender = bool(re.search(
            r"\b(?:HOMBRE(?:S)?|MASCULINO|MALE|MEN)\b\s*(?:Y|E|AND|&|/)\s*"
            r"\b(?:MUJER(?:ES)?|FEMENINO|FEMALE|WOMEN)\b|"
            r"\b(?:MUJER(?:ES)?|FEMENINO|FEMALE|WOMEN)\b\s*(?:Y|E|AND|&|/)\s*"
            r"\b(?:HOMBRE(?:S)?|MASCULINO|MALE|MEN)\b",
            text,
        ))
        natural_all = bool(
            re.search(r"\bTOD(?:OS|AS)\b.{0,24}\bGENEROS?\b", text)
            or re.search(r"\b(?:CUALQUIER|TODO)\s+GENERO\b", text)
            or re.search(r"\bSIN\s+(?:DISTINCION|RESTRICCION)\s+DE\s+GENERO\b", text)
        )
        if paired_gender or natural_all:
            result.extend((1, 2))
            continue
