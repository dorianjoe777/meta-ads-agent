...
TOOL_DEFINITIONS = [
    ...
    ("stage_lead_form", "Design a native Meta Lead Ads / Instant Form with the buyer and return exact manual Ads Manager creation steps. This does not create or approve a Meta form. After the buyer says it is created, call list_lead_forms, resolve its real lead_gen_form_id, and use that ID in campaign staging."),
    ("stage_campaign", "Create or stage a full Meta campaign stack. If the buyer requested a fully PAUSED/no-spend campaign and required details are present, the backend may create the campaign/ad set/ad objects in Meta immediately without a second approval; activation/spend remains approval-protected. Include up to three prioritized success_metrics/key_results/KPIs such as ROAS, cost per purchase, and cost per initiate checkout when known. An Image 2 result can be handed off as creative_asset_id=<returned asset_id>; it is a protected local asset, not a public URL. A previously archived buyer creative can be handed off as content_asset_ids=[<saved asset id>]; the backend resolves its approved local file before validation. For static website images pass use_direct_publishing=true so the backend creates the native Page post and ad safely. Natural ads:[{image_path|creative_asset_id|content_asset_ids: ...}] input is accepted too. For video website ads that should be finished in Ads Manager, pass manual_creative_completion=true to create campaign/ad set only, or create_placeholder_ad=true with placeholder_ad_count and placeholder_ad_names to create paused static-placeholder ads that the buyer replaces with video. For native WhatsApp click-to-message campaigns, pass message_destination=WHATSAPP plus the approved prefilled text. Do not guess the phone identifier: the backend resolves the current Page-linked number from live Meta state and existing native ad sets, then uses OUTCOME_ENGAGEMENT, CONVERSATIONS, destination_type=WHATSAPP and the Page/WhatsApp promoted object. A number connected through the WhatsApp Business mobile app is supported; Cloud API/WABA is not mandatory. Static WhatsApp creatives use the same inline link_data + WHATSAPP_MESSAGE structure as Ads Manager and use the primary Ads API app/token first. When that app is Live, no second app or manually-created dark post is required. Retry with a separate Live publishing token only after Meta explicitly reports the primary app is in Development and only if the fallback has ads_management, ads_read and selected-ad-account access. Treat error 1487246 as a wrong or stale number and re-resolve live Meta state; never silently switch the campaign to Traffic. Offer a wa.me TRAFFIC fallback only as an explicit buyer-approved compromise because it changes Meta's optimization. For detailed interests, recommend Advantage+ audience suggestions by default (`targeting_mode=advantage_plus`, flag 1) for prospecting/small budgets; use `targeting_mode=manual` (flag 0) only when strict control is strategically justified or explicitly requested. Never omit the flag when interests are present. Never claim Meta supports empty ads without creatives. Targeting is double-checked server-side: use only decimal interest IDs returned by search_meta_targeting, preserve explicit countries and age bounds, and stop before any Graph mutation if Meta cannot confirm the current catalog entry."),
