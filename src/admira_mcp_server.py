...
TOOL_DEFINITIONS = [
    ...
    ("codex_image_generate", "Generate logo candidates, brand explorations, moodboards, brand samples, standalone assets, organic social posts, approved Meta Ads raster images, or storyboard media through Codex/Image only when the buyer semantically asks to create or revise image media. During branding, use purpose=logo, brand_exploration, moodboard, or brand_sample and show the actual result for natural approval before saving it as official. Organic and paid production require durable buyer-confirmed branding. A campaign goal, budget, destination, unanswered creative-choice question, statement that no creative exists, or the model's own offer to generate one is not a generation request. For a paid campaign, first present and obtain natural correction/approval of the commercial angle, exact primary text, distinct title, CTA/destination message, and visual concept; pass the exact active child offer in product_guide or ad_brief. Always send a self-contained request with the exact active topic/offer, desired composition, format, CTA decision, and reference when relevant. For hybrid real-media work, first refine even a short natural buyer request into a rich semantic art direction using the confirmed active offer, parent branding, audience, objective, format, requested extra details, exact/proposed on-image text, and ordered assets. Do not merely copy an incomplete buyer sentence, invent business facts, or force a fixed template; preserve meaning while allowing fresh composition on every variation. The backend safely restores confirmed context if an optional field is sparse. Ordinary inspiration may use reference_image_paths/content_asset_ids. Whenever buyer-owned real photos must remain exact and visible in the final design, use the hybrid real_media contract instead of asking Image 2 to reproduce them. Set layout_intent=hero|before_after|services|collage|freeform, visual_direction, structured text_content, and an ordered real_media list of up to six slots. Image 2 creates the dynamic overlay; the backend inserts protected photos and the official logo afterward. For attached buyer photos, first inspect the batch and call mcp_admira_save_content_asset to classify/save each file (pending_agent_review is only an archival state), then use the returned asset IDs in real_media in this or the next tool turn; do not leave clearly assigned photos pending or route them through ordinary image generation. This is semantic sequencing, not a keyword filter or extra approval barrier. Approved brand-scoped style references are attached automatically. Use explicit for one task-scoped reference selected for the current creative; it is merged with brand references but never overrides confirmed brand rules or exact offer facts. Use none only when the buyer explicitly wants no design references for that generation. On any retry, preserve the same layout and every real_media slot unless the buyer changed the request. Real photos and logos are never style references. Brand colors and the requested logo color mode/position must be passed so keyed placeholders avoid brand hues."),
    ("list_recent_creatives", "List generated images from the buyer's three-day recovery window. Use for natural date references and whenever the buyer or agent is unsure whether a relevant campaign creative already exists. Inspect matches, then show the chosen file with MEDIA:file_path; never require an asset ID or treat file existence as campaign approval."),
...
TOOL_INPUT_SCHEMAS = {
    ...
    "save_content_asset": {
        ...
        "properties": {
            ...
            "reference_scope": _string(
                "Only for category=style_reference: task is one-off and must be explicitly selected for a creative; brand means the buyer approved it as durable branding direction and it is attached automatically to future creatives. Defaults safely to task.",
                enum=("task", "brand"),
            ),
        },
        "anyOf": [
    ...
    "codex_image_generate": {
        ...
        "additionalProperties": True,
        "properties": {
            "request": _string("Self-contained image request with exact active offer/topic, visual concept, desired message, format, and CTA decision. A brief buyer phrase must be semantically refined with confirmed context before the call, not copied as the entire art direction."),
            ...
            "layout_intent": _string("Hybrid real-media composition: hero, before_after, services, collage, or freeform.", enum=("hero", "before_after", "services", "collage", "freeform")),
            "visual_direction": _string("Refined natural-language visual direction preserving the buyer's instructions and adding confirmed brand/offer composition, hierarchy, variation, extra requested details, and safe areas. It is not a fixed template."),
            "text_content": {
                "type": "object",
                "additionalProperties": True,
                "description": "Structured text that Image 2 may place in the overlay. Preserve supplied facts/wording; when draft text is needed, use only confirmed active-offer facts and never invent claims.",
                "properties": {
                    "title": _string("Main title or headline to render."),
                    "subtitle": _string("Supporting subtitle or benefit."),
                    "body": _string("Optional supporting body copy."),
                    "bullets": _strings("Ordered benefits, features, or service details to render around or over the real-media windows."),
                    "cta": _string("Call to action text, or empty when no CTA is wanted."),
                    "labels": _strings("Ordered labels for real-media slots, such as ANTES and DESPUÉS."),
                },
            },
            "real_media": {
                "type": "array",
                "minItems": 1,
                "maxItems": 6,
                "description": "Ordered protected real photos. Each slot maps one exact source to one keyed placeholder; supports before/after, multiple services, and collages.",
                "items": {
                    "type": "object",
                    "additionalProperties": False,
                    "properties": {
                        "slot_id": _string("Stable unique slot identifier, for example before, after, service_engine, or service_detail."),
                        "content_asset_id": _string("Durable buyer-owned content asset ID."),
                        "asset_id": _string("Alias for the durable buyer-owned content asset ID."),
                        "file_path": _string("Product-validated local file path for a buyer-owned photo; never accept an arbitrary unvalidated path."),
                        "role": _string("Semantic role of this photo.", enum=("hero", "before", "after", "service", "collage_item", "supporting")),
                        "label": _string("Human-facing label to associate with this slot."),
                    },
                    "required": ["slot_id"],
                    "anyOf": [{"required": ["content_asset_id"]}, {"required": ["asset_id"]}, {"required": ["file_path"]}],
                },
            },
            "style_reference": {
                "type": "object",
                "additionalProperties": False,
                "description": "Graphic-design reference policy. When omitted, every approved brand-scoped reference is attached automatically. explicit adds one task reference while retaining brand references. none disables references for this generation only; pool is a compatibility alias for the persistent brand set.",
                "properties": {
                    "mode": _string("Reference mode. Prefer omission for normal automatic brand references; explicit for a one-task reference; none only for an explicit buyer request to suppress references.", enum=("none", "pool", "explicit")),
                    "asset_id": _string("Exact saved style-reference asset ID when mode=explicit."),
                    "file_path": _string("Product-validated exact style-reference path when mode=explicit."),
                },
                "required": ["mode"],
                "anyOf": [
                    {"properties": {"mode": {"const": "none"}}},
                    {"properties": {"mode": {"const": "pool"}}},
                    {"properties": {"mode": {"const": "explicit"}}, "anyOf": [{"required": ["asset_id"]}, {"required": ["file_path"]}]},
                ],
            },
            "brand_colors": _strings("Brand palette. The compositor chooses saturated placeholder/key colors outside these hue families."),
            "include_logo": _boolean("Whether to insert the protected official logo programmatically after Image 2 generates the overlay."),
            "logo_color_mode": _string("Protected logo rendering variant.", enum=("original", "white", "black", "brand_primary", "brand_secondary", "auto_contrast")),
            "logo_position": _string("Logo placement in the final composition: auto, top_left, top_right, bottom_left, or bottom_right."),
        },
