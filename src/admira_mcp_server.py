...
TOOL_DEFINITIONS = [
    ...
    ("stage_campaign", "Stage a full campaign stack for approval. Include up to three prioritized success_metrics/key_results/KPIs such as ROAS, cost per purchase, and cost per initiate checkout when known. For static-image ads, pass use_direct_publishing=true when the buyer/setup should use Publicación directa; if connected, the backend will create an unpublished Page post during approval execution and then create the ad from its object_story_id. A staged payload can show has_object_story_id=false before approval because the object_story_id is produced at execution time; check creative_route/will_create_object_story_id/direct_publishing_plan instead."),
