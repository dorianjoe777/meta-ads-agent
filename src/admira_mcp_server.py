...
TOOL_DEFINITIONS = [
    ...
    ("stage_campaign", "Create or stage a full Meta campaign stack. If the buyer requested a fully PAUSED/no-spend campaign and required details are present, the backend may create the campaign/ad set/ad objects in Meta immediately without a second approval; activation/spend remains approval-protected. Include up to three prioritized success_metrics/key_results/KPIs such as ROAS, cost per purchase, and cost per initiate checkout when known. An Image 2 result can be handed off as creative_asset_id=<returned asset_id>; it is a protected local asset, not a public URL. A previously archived buyer creative can be handed off as content_asset_ids=[<saved asset id>]; the backend resolves its approved local file before validation. For static images always pass use_direct_publishing=true: the backend creates the dark/unpublished Page post using the live publishing app, with copy, CTA and destination, then creates the ad from object_story_id. Natural ads:[{image_path|creative_asset_id|content_asset_ids: ...}] input is accepted too. For video website ads that should be finished in Ads Manager, pass manual_creative_completion=true to create campaign/ad set only, or create_placeholder_ad=true with placeholder_ad_count and placeholder_ad_names to create paused static-placeholder ads that the buyer replaces with video. For WhatsApp click-to-message campaigns, pass message_destination=WHATSAPP and whatsapp_phone_number_id when available; the backend uses Meta's CONVERSATIONS optimization goal and includes the connected Page/WhatsApp promoted object instead of any stale web-conversion pixel. If Meta returns phone-not-linked (1487246) or the number is WhatsApp Business App rather than a Marketing-API/WABA number, do not retry the native route: use the supported fallback with destination_type=WEBSITE, objective=TRAFFIC, optimization_goal=LINK_CLICKS or LANDING_PAGE_VIEWS, and a wa.me URL containing the approved prefilled text. For static fallback ads, the backend keeps a real PHOTO dark post and carries that URL on the existing-post creative, so the image is not lost in a generic link-share object; tell the buyer that native CONVERSATIONS requires linking a Cloud API/WABA number to the ad account. For detailed interests, recommend Advantage+ audience suggestions by default (`targeting_mode=advantage_plus`, flag 1) for prospecting/small budgets; use `targeting_mode=manual` (flag 0) only when strict control is strategically justified or explicitly requested. Never omit the flag when interests are present. Never claim Meta supports empty ads without creatives. Targeting is double-checked server-side: use only decimal interest IDs returned by search_meta_targeting, preserve explicit countries and age bounds, and stop before any Graph mutation if Meta cannot confirm the current catalog entry."),
