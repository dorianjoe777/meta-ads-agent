...
def reset_history(chat_id):
    ...
    write_json_file(HISTORY_FILE, histories, ensure_ascii=False)
    # Conversation reset is intentionally narrower than installation reset:
    # discard temporary campaign-edit scope, but preserve Facebook OAuth and
    # the buyer's active ad account/Page selection.
    from campaign_editing import reset_conversation_edit_context
    reset_conversation_edit_context(str(chat_id))
...
def handle_text_approval(config, chat_id, text, dashboard, send=True, reply_approval_id=""):
    ...
    if not pending:
        # Approval words also describe approved creatives/messages inside
        # ordinary campaign briefs. With no pending action there is nothing
        # for this pre-model router to decide, so let Hermes interpret the
        # complete natural-language message.
        return None
    decision = text_approval_decision(text)
    if not decision:
        return None
    exact_reference = reply_approval_id or extract_approval_id(text)
    # A long message that merely mentions "approved" is a normal request,
    # unless Telegram tells us it directly replies to an approval card (or it
    # carries the exact hidden action id). Hermes remains free to understand
    # every natural formulation; only concise card-context replies use this
    # deterministic fast path.
    if not exact_reference and len(str(text or "").split()) > 12:
        return None
...
def agent_payload(message, chat_id, language, image_paths=None):
    ...
    return {
        ...
        "oauth_workspace": {
            ...
            "pages": oauth.get("pages") or [],
            "businesses": oauth.get("businesses") or [],
        },
        "channel": "telegram",
    }
...
def handle_text(config, chat_id, text, send=True, image_paths=None, reply_approval_id=""):
    ...
    if command in {"/start", "/ayuda", "/help"}:
        reply = help_message()
    elif command in {"/nuevo", "/new", "/reset", "/restart"}:
        reset_history(chat_id)
        reply = "Listo. Reinicié solo el contexto de esta conversación; tus conexiones de Facebook, cuentas y Página activas siguen guardadas. ¿Qué quieres trabajar ahora?"
...
def handle_callback(config, query):
    ...
    if action in {"meta_account", "meta_page"}:
        # Legacy inline workspace buttons may remain visible in an older chat.
        # They are deliberately non-authoritative: the only supported mutation
        # path is the protected two-number Page/ad-account reply.
        dashboard = load_dashboard_module()
        callback_answer(config, callback_id, "Responde con los dos números en el chat.")
        try:
            dashboard._send_meta_oauth_account_picker(
                config,
                dashboard._meta_oauth_connection(),
                str(chat_id),
            )
        except Exception:
            send_message(
                config,
                chat_id,
                "Ese selector anterior ya no es válido. Pide nuevamente la lista y responde solo con Página y cuenta, por ejemplo: 1, 8.",
            )
        return {"handled": True, "type": "legacy_meta_workspace_callback_rejected"}
