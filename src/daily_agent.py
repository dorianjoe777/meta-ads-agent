...
def command_for_pending(item):
    ...
    if item.get("type") == "budget_change":
        amount = payload.get("recommended_budget", payload.get("new_budget", 0))
        currency = str(payload.get("account_currency") or "").strip().upper()
        api_amount = meta_budget_api_amount(amount, currency)
        return ["set_budget", payload.get("target_type", "adset"), payload.get("target_id", payload.get("campaign_id")), api_amount]
    if item.get("type") == "campaign_edit":
        return ["campaign_edit", payload]
...
def native_campaign_creative_link(campaign, ad_plan, destination, message_destination="", lead_gen_form_id=""):
    ...
    if explicit:
        return explicit
    if bool((ad_plan or {}).get("on_meta_destination")) and page_id:
        # Image awareness/engagement creatives need link_data to preserve a
        # headline. Pointing at the selected Facebook Page keeps the entire
        # destination inside Meta and avoids an invented external URL.
...
def native_campaign_cta(ad_plan, link="", message_destination="", lead_gen_form_id=""):
    ...
    if not link:
        return ""
    if bool((ad_plan or {}).get("on_meta_destination")) and not str((ad_plan or {}).get("cta") or "").strip():
        return ""
...
def instagram_direct_identity_missing(message_destination="", destination=None):
    """Block native Instagram Direct before Graph when no professional actor is bound."""
    return (
        SocialFlowClient.normalize_message_destination(message_destination) == "INSTAGRAM_DIRECT"
        and not str((destination or {}).get("instagram_actor_id") or "").strip()
    )
...
META_SUPPORTED_BUDGET_CURRENCIES = {
    # Current AdAccount.Currency values generated by Meta's Business SDK.
    # Unknown/future currencies must be added with a verified offset instead
    # of silently inheriting USD's x100 behavior.
    "AED", "ARS", "AUD", "BDT", "BOB", "BRL", "CAD", "CHF", "CLP", "CNY",
    "COP", "CRC", "CZK", "DKK", "DZD", "EGP", "EUR", "GBP", "GTQ", "HKD",
    "HNL", "HUF", "IDR", "ILS", "INR", "ISK", "JPY", "KES", "KRW", "LKR",
    "MOP", "MXN", "MYR", "NGN", "NIO", "NOK", "NZD", "PEN", "PHP", "PKR",
    "PLN", "PYG", "QAR", "RON", "SAR", "SEK", "SGD", "THB", "TRY", "TWD",
    "UAH", "USD", "UYU", "VND", "ZAR",
}
...
def meta_budget_api_amount(amount, currency=""):
    ...
    if normalized_currency not in META_SUPPORTED_BUDGET_CURRENCIES:
        return 0
...
def structured_targeting_locations(value):
    """Normalize live Meta location objects without converting them to countries."""
    ...
    for item in values:
        ...
        if location_type == "country":
            key = (country_code or key).upper()
            country_code = key
        if not key or location_type not in {"country", "city", "region"}:
            continue
        identity = (location_type, key.upper() if location_type == "country" else key)
        if identity in seen:
            continue
        seen.add(identity)
        clean = {
            "key": identity[1],
            "id": identity[1],
            "name": str(item.get("name") or item.get("label") or identity[1]).strip(),
            "type": location_type,
        }
        if country_code:
...
def targeting_geo_signature(targeting):
    """Return the exact Graph geography, excluding display-only location_types."""
    geo = targeting.get("geo_locations") if isinstance(targeting, dict) else {}
    if not isinstance(geo, dict):
        return {}
    signature = {}
    countries = normalize_location_codes(geo.get("countries"), default=[])
    if countries:
        signature["countries"] = sorted(set(countries))
    for key in ("cities", "regions", "zips", "geo_markets"):
        values = geo.get(key)
        values = values if isinstance(values, list) else ([values] if values else [])
        ids = []
        for item in values:
            value = str((item.get("key") or item.get("id")) if isinstance(item, dict) else item or "").strip()
            if value:
                ids.append(value)
        if ids:
            signature[key] = sorted(set(ids))
    cluster_ids = geo.get("location_cluster_ids")
    cluster_ids = cluster_ids if isinstance(cluster_ids, list) else ([cluster_ids] if cluster_ids else [])
    cluster_ids = sorted({str(value).strip() for value in cluster_ids if str(value).strip()})
    if cluster_ids:
        signature["location_cluster_ids"] = cluster_ids
    return signature
...
def targeting_for_social(targeting):
    ...
    raw_locations = targeting.get("locations") or targeting.get("targeting_locations")
    direct_selected_locations = structured_targeting_locations(raw_locations)
    country_source = (
        targeting.get("countries")
        or targeting.get("targeting_countries")
        or (raw_locations if not direct_selected_locations else None)
    )
    countries = normalize_location_codes(country_source, default=[])
    ...
    if geo_locations is None:
        geo_locations = {"countries": countries} if countries else {}
    selected_locations = meta_targeting.get("locations") if isinstance(meta_targeting, dict) else []
    if not selected_locations:
        selected_locations = direct_selected_locations
    if isinstance(selected_locations, list) and selected_locations:
        ...
        # Never replace an invalid or empty structured selection with US.
        # Preflight rejects an empty geography before the first Graph write.
    spec = {
...
def targeting_placement_signature(targeting):
    """Return automatic or the exact explicit Meta placement selection."""
    if not isinstance(targeting, dict):
        return {"automatic": True}
    signature = {}
    for key in PLACEMENT_TARGETING_KEYS:
        values = targeting.get(key)
        values = values if isinstance(values, list) else ([values] if values else [])
        normalized = sorted({str(value or "").strip().lower() for value in values if str(value or "").strip()})
        if normalized:
            signature[key] = normalized
    if not signature:
        return {"automatic": True}
    return {"automatic": False, **signature}


def targeting_needs_live_verification(targeting):
    return (
        bool(targeting_geo_signature(targeting))
        or bool(targeting_interest_ids(targeting))
        or targeting_advantage_value(targeting) is not None
        or bool(targeting_placement_signature(targeting))
    )
...
def verify_adset_targeting_result(requested_targeting, result):
    ...
    requested_geo = targeting_geo_signature(requested_targeting)
    actual_geo = targeting_geo_signature(actual)
    requested_placements = targeting_placement_signature(requested_targeting)
    actual_placements = targeting_placement_signature(actual)
    ...
    geo_matches = bool(requested_geo and requested_geo == actual_geo)
    placements_match = requested_placements == actual_placements
    confirmed = bool(read_ok and not missing_ids and advantage_matches and geo_matches and placements_match)
    return {
        "ok": confirmed,
        "confirmed": confirmed,
        "source": "meta_live",
        ...
        "requested_geo_locations": requested_geo,
        "persisted_geo_locations": actual_geo,
        "geo_locations_match": geo_matches,
        "requested_placements": requested_placements,
        "persisted_placements": actual_placements,
...
def validate_campaign_targeting_before_meta(campaign, client):
    ...
    for index, adset in enumerate(campaign.get("ad_sets") or []):
        ...
        raw_direct_locations = targeting.get("locations") or targeting.get("targeting_locations")
        raw_direct_location_items = (
            raw_direct_locations
            if isinstance(raw_direct_locations, list)
            else [raw_direct_locations] if raw_direct_locations else []
        )
        direct_selected_locations = structured_targeting_locations(raw_direct_locations)
        raw_location_objects = [item for item in raw_direct_location_items if isinstance(item, dict)]
        # A numeric Meta key without a city/region type is not a country.
        # Falling back to an accompanying country code would silently broaden
        # the buyer's audience, so reject it before create_campaign.
        if raw_location_objects and len(direct_selected_locations) != len(raw_location_objects):
            return {
                "ok": False,
                "code": "targeting_preflight_failed",
                "validations": [{
                    ...
                    "errors": [{
                        "field": "locations",
                        "code": "targeting_location_structure_incomplete",
                        "message": "Una ubicación de Meta perdió su tipo city/region/country; no se amplió al país.",
                    }],
                }],
                "message": "Una ubicación exacta de Meta llegó incompleta; no se creó ningún objeto ni se amplió la audiencia.",
            }
        selected_locations = meta_targeting.get("locations") or direct_selected_locations
        ...
        if not locations:
            return {
                ...
                "validations": [{
                    ...
                    "errors": [{
                        "field": "locations",
                        "code": "targeting_location_missing",
                        "message": "La campaña no contiene una ubicación verificable; no se creó ningún objeto.",
                    }],
                }],
                "message": "La campaña no contiene una ubicación verificable; no se creó ningún objeto.",
            }
...
def execute_campaign_creation(path, client, approved=False, prior_result=None):
    ...
    if instagram_direct_identity_missing(message_destination, destination):
        missing.append("linked professional Instagram account ID for Instagram Direct")
...
def execute_pending(item, client):
    ...
    if item.get("type") == "budget_change":
        budget_payload = item.get("payload") if isinstance(item.get("payload"), dict) else {}
        if not str(budget_payload.get("account_currency") or "").strip() or not str(budget_payload.get("budget_confirmation") or "").strip():
            return {
                "ok": False,
                "blocked": True,
                "error": "legacy_budget_approval_requires_restage",
                "reply": "Vuelve a preparar este cambio indicando el monto y la moneda exactos; no ejecutaré una aprobación antigua sin contexto monetario.",
            }
    ...
    if command[0] == "set_budget":
        ...
        verification = {"ok": False, "reason": "budget_verification_unavailable"}
        if result.get("executed") and result.get("returncode") in {0, None} and hasattr(client, "get_graph"):
            live = client.get_graph(command[2], {"fields": "id,name,daily_budget,currency"})
            body = live.get("body") if isinstance(live, dict) and isinstance(live.get("body"), dict) else {}
            try:
                actual_api_amount = int(body.get("daily_budget"))
            except (TypeError, ValueError):
                actual_api_amount = -1
            verification = {
                "ok": bool(live.get("ok")) and actual_api_amount == int(command[3]),
                "target_id": command[2],
                "expected_api_amount": int(command[3]),
                "actual_api_amount": actual_api_amount,
                "account_currency": str((item.get("payload") or {}).get("account_currency") or body.get("currency") or "").strip().upper(),
                "buyer_amount": float((item.get("payload") or {}).get("new_budget") or 0),
                "budget_confirmation": str((item.get("payload") or {}).get("budget_confirmation") or ""),
            }
        if not verification.get("ok"):
            return {
                "ok": False,
                ...
                "blocked": True,
                "error": "budget_readback_mismatch",
                "mutation_result": result,
                "verification": verification,
            }
        result = {**result, "ok": True, "budget_verified": True, "verification": verification}
    elif command[0] == "campaign_edit":
        edit_payload = command[1] if isinstance(command[1], dict) else {}
        result = execute_campaign_edit(
            edit_payload,
            client,
...
def reject(approval_id, reason=""):
    ...
    for item in pending:
        ...
        if item.get("type") == "campaign_edit":
            edit_payload = item.get("payload") if isinstance(item.get("payload"), dict) else {}
            if edit_payload.get("draft_path"):
                mark_draft_status(
                    edit_payload.get("draft_path"),
                    "rejected",
                    result={
