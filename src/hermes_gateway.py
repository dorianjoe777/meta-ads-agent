...
def gateway_prompt(language="es", communication_style="simple", ad_experience_level=""):
    ...
    if str(language or "es").lower().startswith("en"):
        return (
            ...
            " FACEBOOK-OAUTH DELIVERY OVERRIDE: never depend on an inline Telegram button. When `mcp_admira_start_meta_oauth_connection` succeeds, the product sends the secure authorization URL as ordinary visible message text; tell the buyer to open that URL and reply naturally when finished. On the next user turn call `mcp_admira_get_meta_oauth_workspaces` and list every returned ad account and publishable Page with short numbers/names in ordinary text. Obtain an explicit natural-language choice for both assets. If the buyer names only an account, ask which Page to use in the next text message; if they name only a Page, ask which account to use. Never auto-select the first Page or infer the missing half of the pair. Only then call `mcp_admira_select_meta_oauth_workspace` with both matching IDs. Success is authoritative only when the tool returns `selected: true` and `verified_persisted: true`. Do not call the selection tool merely because OAuth connected and do not ask the buyer to click an account/Page button."
    ...
    return (
        ...
        " OVERRIDE DE ENTREGA FACEBOOK-OAUTH: nunca dependas de un botón inline de Telegram. Cuando `mcp_admira_start_meta_oauth_connection` tenga éxito, el producto envía la URL segura como texto visible normal; dile al comprador que abra esa URL y responda naturalmente cuando termine. En el turno siguiente llama `mcp_admira_get_meta_oauth_workspaces` y lista mediante texto normal todas las cuentas publicitarias y Páginas publicables devueltas, con números y nombres breves. Obtén una elección explícita en lenguaje natural para ambos activos. Si el comprador menciona solo la cuenta, pregunta por texto cuál Página quiere usar; si menciona solo la Página, pregunta cuál cuenta. Nunca selecciones automáticamente la primera Página ni infieras la mitad faltante del par. Solo entonces llama `mcp_admira_select_meta_oauth_workspace` con ambos IDs correspondientes. El éxito solo es autoritativo cuando la herramienta devuelve `selected: true` y `verified_persisted: true`. No llames la selección solo porque OAuth terminó ni pidas hacer clic en botones de cuenta/Página."
        f" {style_instruction} {experience_instruction}"
    )
